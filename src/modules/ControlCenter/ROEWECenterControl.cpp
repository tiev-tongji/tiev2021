...
void ROEWEControl::get_can_info(){
	...
	while(1){
		for(int i = 0; i < rec; i++){
			nbytes = read(can_fd, &frame[i], sizeof(frame[i]));
            // TODO: 等待稍微改动后，看状态如何
			//switch((frame + i)->can_id){
			switch((frame+i)->can_id){
                case 0x18B:
                    this->get_m_EPS_HSC_FrP01(&frame[i], &car_angle);
                    car_angle_ = car_angle;
                    DEBUG("CANINFO: car_angle ==> " << car_angle_);
                    break;
                case 0x302:
                    this->get_m_VCU2MAB_2(&frame[i], &car_speed);
                    car_speed_ = car_speed;
                    DEBUG("CANINFO: car_speed ==> " << car_speed_);
                    break;
				case 0x312:
					this->get_m_EPS2VMS(&frame[i], &veh_control_mode);
...
void ROEWEControl::send_can_info(){
	...
	while(true){
		...
		this->send_m_VMS2EPS(&frame[0], angle_torque);
		this->send_m_MAB2VCU(&frame[1], speed_torque);
...
// 角度控制输出
STATE ROEWEControl::send_m_VMS2EPS(can_frame * frame, float& angle_torque, unsigned char enable_control){
    frame->can_id = 0x310;
	frame->can_dlc = 8;
...
// 速度控制输出
STATE ROEWEControl::send_m_MAB2VCU(can_frame * frame, float& speed_torque, unsigned char enable_control){
	frame->can_id = 0x313;
	frame->can_dlc = 8;
...
// 方向盘转向获取
STATE ROEWEControl::get_m_EPS_HSC_FrP01(can_frame *frame, float* car_angle){
...
// 车速获取
STATE ROEWEControl::get_m_VCU2MAB_2(can_frame * frame,  float* car_speed){
...
STATE ROEWEControl::get_m_EPS2VMS(can_frame * frame, unsigned char* control_mode){
	EPS2VMS m_EPS2VMS;
