...
private:
    // 方向盘转向获取
    STATE get_m_EPS_HSC_FrP01(can_frame *frame, float* car_angle);
    // 车速获取
    STATE get_m_VCU2MAB_2(can_frame * frame, float* car_speed);
    // 档位启停控制信号获取
    STATE get_m_EPS2VMS(can_frame * frame, unsigned char* control_mode);
    // TODO: 暂时无用
    STATE get_m_VCU2MAB_3(can_frame * frame) {return CC_OK;}
    STATE get_m_VCU2MAB_4(can_frame * frame) {return CC_OK;}
    STATE get_m_VCU2MAB_5(can_frame * frame) {return CC_OK;}
    STATE get_m_BSC2MCU(can_frame * frame) {return CC_OK;}

    STATE send_m_VMS2EPS(can_frame * frame, float& angle_torque, unsigned char enable_control = 1);
    STATE send_m_MAB2VCU(can_frame * frame, float& speed_torque, unsigned char enable_control = 1);
private:
    int can_fd = 0;
    // 车辆实际返回
