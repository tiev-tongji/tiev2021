...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        ///////fix direction
        if(prev_speed_sign * veh_info.speed < -1e-1) veh_info.speed = -veh_info.speed;
        if(veh_info.speed > 1) prev_speed_sign = 1;
        else if(veh_info.speed < -1) prev_speed_sign = -1;
        else prev_speed_sign = 0;
        cout << "Actual Velocity:" << veh_info.speed << "Desired Velocity:" << veh_pc_control_info.speed << endl;
       ...
       // speed_pid_control(veh_info.speed, veh_pc_control_info.speed, params, &is_break, &speed_torque);
        ...
        angle_pid_control(veh_info, veh_pc_control_info.angle, K, params, &angle_torque);
