...
STATE speed_pid_control(const float &veh_speed, float &desired_speed,
                        float &angle_pitch, const control_params_t &params,
                        bool *is_break, float *control_output) {
  float FF_valve = 2;
  ...
  // TODO: 速度超过一点点如何处理？
  // INFO("------------------------------------------P_speed:" << P_speed << "
  // veh_speed:" << veh_speed << " veh_break_enable:" << veh_break_enable);
  ...
  // 需要进行减速处理
    ...
    *control_output =
        -(feedfoward_contribute + P_contribute + I_contribute + D_contribute);

    // INFO("BREAK INFO ==> P: " << P_contribute << ", " << "I: " <<
    // I_contribute << ", " << "D: " << D_contribute);
    if (PID_DEBUG)
...
STATE angle_pid_control(const veh_info_t &veh_info, float &desired_angle,
                        float Cur, const control_params_t &params,
                        float *control_output) {

  // 正常模式计算
  float k = 1;
  float P_angle = desired_angle - veh_info.angle;
  // float P_angle = veh_info.angle - desired_angle;
  ...
  if (veh_info.speed <= 20) {
    ...
    FF_contribute = L * Cur;
    // + Kv * veh_info.speed * veh_info.speed * Cur -
    //                 params.steer_FF * (-L_r * Cur +
    //                                    m_r / (2 * C_alpha_r) * veh_info.speed
    //                                    *
    //                                        veh_info.speed * Cur);
  } else {
    ...
    DEBUG("ANGLE Curvature INFO ==> curvature:" << Cur);
    FF_contribute = L * Cur;
    // + Kv * veh_info.speed * veh_info.speed * Cur -
    //                 params.steer_FFH * (-L_r * Cur +
    //                                     m_r / (2 * C_alpha_r) *
    //                                     veh_info.speed *
    //                                         veh_info.speed * Cur);
  }
  ...
  if (PID_DEBUG) {
    DEBUG("ANGLE INFO ==> P: " << P_contribute << ", "
                               ...
                               << "D: " << D_contribute
                               << " FF: " << FF_contribute);
    DEBUG("ANGLE INFO ==> desired_angle:" << desired_angle
                                          << " veh_ang:" << veh_info.angle
                                          << " P_angle:" << P_angle);
  }
