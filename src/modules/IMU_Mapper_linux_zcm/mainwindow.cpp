...
void MainWindow::showInfo() {
  ...
  QString rtk_mHeading =
      "mHeading: " + QString::number(NAVI_local.mHeading * 180.0 / M_PI, 10, 7);
  ...
  QString slam_mHeading =
      "mHeading: " +
      QString::number(SLAMLOC_local.mHeading * 180.0 / M_PI, 10, 7);
  ...
  if (record_roadpoint) {
    ...
    if (event_record == true) {
      ...
      double dist = (data_temp.utmX - data_rtk_for_slam_test.utmX) *
                        (data_temp.utmX - data_rtk_for_slam_test.utmX) +
                    (data_temp.utmY - data_rtk_for_slam_test.utmY) *
                        (data_temp.utmY - data_rtk_for_slam_test.utmY);
      double delta_yaw =
          abs(data_temp.heading - data_rtk_for_slam_test.heading);
      cout << "rtk-slam distance: " << sqrt(dist)
           << "delta mheading: " << delta_yaw << endl;
  ...
  ui->label_3->setText("RTKStatus: " +
                       QString::number(NAVI_local.mRTKStatus, 10));
  ...
  ui->label_12->setText("SLAMStatus: " +
                        QString::number(SLAMLOC_local.SLAMStatus, 10));

  string info = "-Mode INFO-> SpeedMode: " + std::to_string(speedmode) +
                "; Mode: " + std::to_string(mode) + "; Opposite Side Mode: " +
                std::to_string(opposite_side_mode) + ";" + "\n" +
                "-Lane INFO-> LaneNum: " + std::to_string(laneno) +
                "; LaneSeq: " + std::to_string(laneseq) + "; LaneWidth: " +
                std::to_string(lanewidth) + ";\n-EventPoiuntNum: " +
                std::to_string(event_point_num) + ";\n-LocalizationMode: " +
                getReceiveModeName(ReceiveMode_) + ";";
...
void MainWindow::save_roadmap() {
  if (record_roadpoint) {
    ...
    QString fileName =
        QFileDialog::getSaveFileName(this, QString::fromLocal8Bit("文件另存为"),
                                     "", tr("Config Files (*.txt)"));
    ofstream f1(fileName.toStdString()); //打开文件用于写,若文件不存在就创建它
    if (!f1)
      return; //打开文件失败则结束运行

    f1 << "Id Lon Lat utmX utmY heading curvature mode SpeedMode EventMode "
          "OppositeSideMode LaneNum LaneSeq LaneWidth"
       << endl;
    for (int i = 0; i < data.size(); i++) {
      f1 << fixed << setprecision(14) << i << " " << data[i].lon << " "
         << data[i].lat << " " << data[i].utmX << " " << data[i].utmY << " "
         << data[i].heading << " " << data[i].curvature << " " << data[i].mode
         << " " << data[i].speedmode << " " << data[i].eventmode << " "
         << data[i].opposite_side_mode << " " << data[i].laneno << " "
         << data[i].laneseq << " ";
...
void MainWindow::SimplifyPointBtn() {
  if (record_roadpoint) {
    ...
    string msg = "point_num: " + std::to_string(num_before) +
                 " -> point_num: " + std::to_string(num_after);
...
MainWindow::roadpoint MainWindow::get_rtk_data(int num,
                                               structNAVINFO navi_data) {
  roadpoint data_temp;
  data_temp.id = num;
...
MainWindow::roadpoint MainWindow::get_slam_data(int num,
                                                structSLAMLOC slam_data) {
...
void MainWindow::PickPoint(std::vector<roadpoint> &data) {
  ...
  while (i < data.size()) {
    ...
    if (rpTemp.eventmode != 0) {
      ...
      // p1 = CoordinateConverter.BLH2XYZ(CurrentPoint.Lat, CurrentPoint.Lon,
      // 0);
      // p2 = CoordinateConverter.BLH2XYZ(rpTemp.Lat, rpTemp.Lon, 0);
      double dist = sqrt((coord2.x - coord1.x) * (coord2.x - coord1.x) +
