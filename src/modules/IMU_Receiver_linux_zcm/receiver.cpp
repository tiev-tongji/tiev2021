    ...
    int start_pts = 20;
     ...
     while (1)
     {
		...
		cout << "mInnHeading:" << nrx->mInnHeading << endl;
 		...
 		// if (rtkstatusnumber >= 250)
		{//rtk lost for 250 periods
			...
			navinfo.mLon = reckoningLongti;
 			...
 			// navinfo.utmX += east_meterIncre_temp;
 			// navinfo.utmY += north_meterIncre_temp;
			...
			if (rtkstatusnumber >= 250){ infoDetail = "RTK lost"; }
			else if (navinfo.mHPOSAccuracy > 0.3){ infoDetail = "RTK precision > 0.3"; }
			else { infoDetail = "RTK lost and RTK precision > 0.3"; }
		}
