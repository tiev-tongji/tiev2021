...
public:
  ...
  scan_segment_p new_segment() {
    ...
    scan_segment_p segment = segments[num_segments - 1];//final one
    if (segment == NULL) {
...
//  std::vector<scan_segment_p> segments[NUM_LASER_BEAMS];
  ...
  void segmentBeams();
  void collectObstacles(std::vector< std::tr1::shared_ptr<TiEV::Obstacle> >* segmented_obstacles);
public:
  LaserSegmenter(VelodyneRings* rings);
  virtual ~LaserSegmenter();

  void segmentVelo(laser_scan_p lscan, std::vector< std::tr1::shared_ptr<TiEV::Obstacle> >* segments);
};
