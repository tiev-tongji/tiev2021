...
{
  float temp = floor(x / grid->resolution);
  *c = temp - grid->map_c0;
  temp = floor(y / grid->resolution);
  *r = temp - grid->map_r0;
}
...
inline void *
grid_get_rc(dgc_grid_p grid, int r, int c)//给定的opencv坐标系下的rows cols找到真实的cell位置
{
