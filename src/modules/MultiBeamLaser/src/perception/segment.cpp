...
void GridSegmenter::findRegions(dgc_perception_map_cells_p obstacles) {
  clearRegions();//regions is obstacle

//  printf("min points: %d\n", settings.segmentation_settings.min_points);
  unsigned short region_id = 1;

  //init a obstacle from region_id
  std::tr1::shared_ptr<GridObstacle>* current_region = new std::tr1::shared_ptr<GridObstacle>(new GridObstacle(region_id, grid_));
  ...
  for (int i=0; i < obstacles->num; i++) {
//    if(obstacles->cell[i]->obstacle > 0)
...
//      current_region->get()->timestamp = timestamp_;
      ...
      int size = current_region->get()->getSize();//cells size
      if (size > settings.segmentation_settings.min_points && size < settings.segmentation_settings.max_points && (*current_region)->getPoints().size() != 0)
      {
        (*current_region)->time_ = timestamp_;
	    //compute the Average robotPose from all the scan_point
	    (*current_region)->robot_pose_when_observed_ = computeAverageRobotPoseForCells((*current_region)->cells_);
	    assert(!std::isnan((*current_region)->robot_pose_when_observed_.x));
        regions_->push_back(*current_region);
        delete current_region;
        current_region = new std::tr1::shared_ptr<GridObstacle>(new GridObstacle(++region_id, grid_));
      } else {
...
void GridSegmenter::segmentGrid(dgc_grid_p grid, dgc_perception_map_cells_p obstacles, vector< std::tr1::shared_ptr<Obstacle> >* regions, double timestamp) {
  grid_ = grid;
  regions_ = regions;
