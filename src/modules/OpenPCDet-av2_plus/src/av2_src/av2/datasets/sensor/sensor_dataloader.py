...
class SensorDataloader:
    ...
    cam_names: Tuple[Union[RingCameras, StereoCameras], ...] = None
    ...
    def _build_sensor_cache(self) -> pd.DataFrame:
        ...
        if self.with_cache and sensor_cache_path.exists():
            if self.logger != None:
                self.logger.info("Cache found. Loading from disk ...")
    ...
    def __getitem__(self, idx: int) -> SynchronizedSensorData:
        ...
        # load_cams_time = time.time()
        
        # load_cams_time -= load_data_time
        # load_data_time -= search_time
        # search_time -= start_time
        # self.logger.info(f'in_search_time: {search_time:.4f}, '
        #                  f'load_data_time: {load_data_time:.4f}, '
        #                  f'load_cams_time: {load_cams_time:.4f}, '
        #                  ) 
    ...
    def _build_synchronization_cache(self) -> pd.DataFrame:
        ...
        """
        if self.logger != None:
            self.logger.info("Building synchronization database ...")
