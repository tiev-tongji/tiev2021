...
def get_lidar_timestamps_from_sensor_folder(sensor_folder_wildcard: str, log_id:str) -> NDArrayInt:
    """Timestamp always lies at end of filename.
    ...
    Returns:
        Numpy array of integers, representing timestamps
    """
    path_generator = glob.glob(f"{sensor_folder_wildcard}/{log_id}/sensors/lidar/*")
    path_generator.sort()

    timestamps: NDArrayInt = np.array([int(Path(jpg_fpath).stem.split(".")[-1]) for jpg_fpath in path_generator])
    return timestamps
...
if __name__ == "__main__":
    ...
    root_dir = "/mnt/hgfs/share-folder/data/argoverse/Argoverse_dataset_sample/train_000/sensor/train"
    camera = "ring_front_center"
    # id = "c6911883-1843-3727-8eaa-41dc8cda8993"
    id = "00a6ffc1-6ce9-3bc3-a060-6006e9893a1a"

    db = SynchronizationDB(root_dir)
    lidar_timestamps = get_lidar_timestamps_from_sensor_folder(root_dir, id)
    for t in lidar_timestamps:
        for cam_name in list(RingCameras) + list(StereoCameras):
            clost_cam_t = db.get_closest_cam_channel_timestamp(t, cam_name, id)
            print("corresponding %s image at %s: %s" % (cam_name, t, clost_cam_t))
        print("-------------------------------------------------")
    db.get_valid_logs()
