...
import av2.datasets.sensor.sensor_dataloader as SD
...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        try:
            # print("check 3       batch = next(dataloader_iter)")
            batch = next(dataloader_iter)
            # print("check 4  DONE batch = next(dataloader_iter)")
        ...
        if fd_time is None:
            fd_time = disp_dict['fd_time']
        else:
            for key, value in disp_dict['fd_time'].items():
                fd_time[key] += value
        lco_stime = time.time()
        loss.backward()
        clip_grad_norm_(model.parameters(), optim_cfg.GRAD_NORM_CLIP)
        optimizer.step()
        lco_time += time.time() - lco_stime
        disp_dict['lco_time'] = time.time() - lco_stime
    ...
    if use_logger_to_record:
        logger.info(f'Details of forward time cost for epoch {cur_epoch}/{total_epochs}: fdtime:{fd_time}, lco_time:{lco_time}')
    if rank == 0:
        pbar.close()
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        # print("check 1    train_model    154   dataloader_iter = iter(train_loader)")
        dataloader_iter = iter(train_loader)
        for cur_epoch in tbar:
            ...
            # print("check 2    166   train_one_epoch")
