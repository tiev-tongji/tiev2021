...
void init(){
	...
	double cornering_stiffness = root["vehiclePara"]["cornering_stiffness"].asDouble();
	...
	double k_lateral_front_high = root["controlPara"]["k_lateral_front_high"].asDouble();
	double k_heading_dev_high = root["controlPara"]["k_heading_dev_high"].asDouble();
	...
	aim_dis_base_backward = root["controlPara"]["aim_dis_base_backward"].asDouble();

	alfa_steering_angle = root["filterPara"]["alfa_steering_angle"].asDouble();
	alfa_velocity= root["filterPara"]["alfa_velocity"].asDouble();

	stanleycontroller.setVehicleParameter(wheel_base, mass, lf, lr, cornering_stiffness, max_wheel_angle);
	stanleycontroller.setControlParameter(k_lateral_front, k_lateral_rear, k_heading_dev, k_lateral_front_high, k_heading_dev_high, flag_dynamic, k_kappa_dev);
...
void StanleyContollerThread(){
	...
	while(1)
	{
		...
		zcmreceiver.getCarStatus(current_velocity, current_steering_angle, current_yawrate);
...
//			getRefPose3(control_path, direction, current_pose, ref_pose);
//			getRefPose(control_path, direction, current_pose, ref_pose);
			getRefPose2(control_path, current_velocity_m_s, aim_dis_k, aim_dis_base, ref_pose);
			ref_velocity = getRefVelocity(control_path, current_velocity_m_s, aim_dis_k, aim_dis_base);
			if(ref_velocity < 0)
				direction = -1;
			else
				direction = 1;
...
//		cout << ref_pose.x << "  " << ref_pose.y << "  " << ref_pose.angle * 180 / PI << "  " << ref_pose.kappa << endl;
        ...
        if(current_velocity < v_switch_high){
			stanleycontroller.computeWheelAngle(ref_pose, current_pose, current_velocity_m_s, direction, current_yawrate, ref_steering_angle);
			ay_max = ay_max_low;
		}
		else{
			stanleycontroller.computeWheelAngleHighSpeed(ref_pose, current_pose, current_velocity_m_s, direction, current_yawrate, ref_steering_angle);  
	       ay_max = ay_max_high;	
		}
		double soft = 0.1;
        double wheel_angle_max = min(atan(wheel_base*ay_max*9.8/(current_velocity_m_s*current_velocity_m_s+soft)),max_wheel_angle);
		ref_steering_angle_filter = filter(ref_steering_angle, ref_steering_angle_filter, alfa_steering_angle);
		ref_velocity_filter = filter(ref_velocity, ref_velocity_filter, alfa_velocity);
		
		end_time = clock();
		used_time = 1000 * (long)(1000.0 * (end_time - start_time) / CLOCKS_PER_SEC);
		if(used_time < 20) usleep((20 - used_time) * 1000); //freq:50Hz

		structCANCONTROL control_msg;
	    double steer_direction = ref_steering_angle_filter > 0 ? 1:-1;
		cout << "steer_direction--------------------------------> " << steer_direction << endl; 
		cout << "wheel_angle_max--------------------------------> " << wheel_angle_max << endl; 

		control_msg.aimsteer = steer_direction*min(fabs(ref_steering_angle_filter), wheel_angle_max) * (180.0 / PI) * 17.4; //rad to deg, add trans ratio
		...
		cout << "aimsteer---------------------------------------> " << control_msg.aimsteer << endl;
		cout << "aimspeed---------------------------------------> " << control_msg.aimspeed << endl;
