...
void Handler::handleAIMPATH(const zcm::ReceiveBuffer *rbuf, const std::string &channel, const structAIMPATH *msg){
    path_mutex.lock();
    tmp_path = *msg;
...
void Handler::handleCANINFO(const zcm::ReceiveBuffer *rbuf, const std::string &channel, const structCANINFO *msg){
    caninfo_mutex.lock();
    tmp_caninfo = *msg;
...
void Handler::handleNAVINFO(const zcm::ReceiveBuffer *rbuf, const std::string &channel, const structNAVINFO *msg){
    navinfo_mutex.lock();
    tmp_navinfo = *msg;
...
bool ZcmReceiver::getControlPath(std::vector<ControlPose>& control_path){
	if(inner_handler.tmp_path.num_points < 1) return false;
...
//		std::cout << "curvature[" << i << "]------------------------->" << buffer_point.kappa << std::endl;
	}
...
bool ZcmReceiver::getCarStatus(double &current_velocity, double &current_steering_angle, double &current_yawrate){
