...
for mode in ['valid']:
    ...
    for i, img_path in enumerate(img_path_list):
        ...
        mask = mask[:, :, 1]
        for ind, clsid in enumerate(classid):
            # pdb.set_trace()
            aa= np.where(mask == clsid)
            labels_num[ind] += len(aa[0])

pdb.set_trace()
labels_all = sum(labels_num)

labels_pr = [lab_num/labels_all for lab_num in labels_num]

labels_wei = [1/math.log(1.02+label_pr) for label_pr in labels_pr]
