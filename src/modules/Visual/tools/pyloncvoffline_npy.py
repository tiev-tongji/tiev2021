...
pic_list = glob.glob('{:s}/**/*.jpg'.format(root), recursive=True)
...
for ii in range(len(pic_list)):
    pic_path = os.path.join(root, 'record_jpg/%04d.jpg'%(ii+1))
    ...
    if True:
        ...
        save_path = os.path.join(root, 'record_npy/%04d.npy'%(ii+1))
