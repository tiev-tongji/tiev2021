...
def plot_one_box(x, img, color=None, label=None, line_thickness=None, direction=None):
    ...
    t_size = cv2.getTextSize(direction, 0, fontScale=tl / 3, thickness=tf)[0]
    c1, c2 = (int(img.shape[1]/2 - t_size[0]/2),img.shape[0]),(int(img.shape[1]/2 + t_size[0]/2),img.shape[0]-t_size[1] - 3)
    cv2.putText(img, direction, (c1[0], c1[1] - 2), 0, tl / 3, [0, 0, 0], thickness=tf, lineType=cv2.LINE_AA)
