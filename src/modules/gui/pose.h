...
struct Pose : public Point2d {
    ...
    Pose(double x_ = 0, double y_ = 0, double ang_ = 0, double k_ = 0,
        ...
        : Point2d(x_, y_), ang(ang_), k(k_), v(v_), a(a_), s(s_), t(t_),
          backward(backward_), utm_position() {}

    inline Point2d getDirectionVec() const {
        return Point2d(cos(ang), sin(ang));
    }

    inline Point2d getNormalVec() const {
        return Point2d(-sin(ang), cos(ang));
    }
    ...
    inline void updateGlobalCoordinate(const Pose& standard_point) {
        ...
        double hd =
            standard_point.utm_position.heading + ang - standard_point.ang;
        while(hd > PI)
            hd -= 2 * PI;
        while(hd <= -PI)
            hd += 2 * PI;
...
struct LaneCenterPoint : public Point2d {
    ...
    LaneCenterPoint(const double x_ = 0, const double y_ = 0,
        ...
        : Point2d(x_, y_), have_priority(have_priority_),
...
struct HDMapPoint : public Pose {
    ...
    HDMapPoint(double utm_x_ = 0, double utm_y_ = 0, double heading_ = 0,
               ...
               RoadDirection direction_ = RoadDirection::STRAIGHT,
               double x_ = 0, double y_ = 0, double ang_ = 0)
        : Pose(x_, y_, ang_, k_) {
        event        = event_;
        mode         = mode_;
    ...
    friend std::ostream& operator<<(std::ostream&     out,
        ...
        out << "HDMapPoint:{utm position:(" << point.utm_position.utm_x << ", "
            ...
            << " block_type=" << point.block_type
            << " lane_num=" << point.lane_num << " lane_seq=" << point.lane_seq
...
struct TiEVLinePoint : public Point2d {
    LineType type;
    TiEVLinePoint(double x_ = -1, double y_ = -1,
                  LineType type_ = LineType::UNKNOWN_LINE)
        : Point2d(x_, y_), type(type_){};
    friend std::ostream& operator<<(std::ostream&        out,
                                    const TiEVLinePoint& point) {
        out << "TiEVLinePoint:{x=" << point.x << " y=" << point.y
            << " type=" << point.type << "}";
        return out;
    }
};
