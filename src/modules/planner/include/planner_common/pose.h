...
struct Pose : public Point2d {
    ...
    bool        backward;
    Pose(double x_ = -1, double y_ = -1, double ang_ = 0, double k_ = 0, double v_ = 0, double a_ = 0, double s_ = 0, double t_ = 0, bool backward_ = false)
        : Point2d(x_, y_), ang(ang_), k(k_), v(v_), a(a_), s(s_), t(t_), backward(backward_), utm_position() {}

    inline Point2d getDirectionVec() const {
    ...
    void updateLocalCoordinate(const Pose& standard_point) {
        ...
        double qy      = (utm_position.utm_y - standard_point.utm_position.utm_y) / GRID_RESOLUTION;
        double sinstdh = sin(stdh), cosstdh = cos(stdh);
        ...
        double ag = utm_position.heading - standard_point.utm_position.heading + standard_point.ang;
        while(ag > PI)
            ag -= 2 * PI;
        while(ag <= -PI)
            ag += 2 * PI;
...
struct HDMapPoint : public Pose {
    ...
    HDMapPoint(double utm_x_ = 0, double utm_y_ = 0, double heading_ = 0, double k_ = 0, HDMapMode mode_ = HDMapMode::NORMAL, HDMapSpeed speed_mode_ = HDMapSpeed::MIDDLE,
               HDMapEvent event_ = HDMapEvent::NONE, BlockType block_type_ = BlockType::BlockAll, int lane_num_ = 0, int lane_seq_ = 0, double lane_width_ = 0,
               RoadDirection direction_ = RoadDirection::STRAIGHT, double x_ = 0, double y_ = 0, double ang_ = 0)
        : Pose(x_, y_, ang_, k_) {
        event        = event_;
        mode         = mode_;
    ...
    friend std::ostream& operator<<(std::ostream& out, const HDMapPoint& point) {
        out << "HDMapPoint:{utm position:(" << point.utm_position.utm_x << ", " << point.utm_position.utm_y << ", " << point.utm_position.heading << ") curve=" << point.k << " mode=" << point.mode
            << " speed_mode=" << point.speed_mode << " event=" << point.event << " block_type=" << point.block_type << " lane_num=" << point.lane_num << " lane_seq=" << point.lane_seq
            << " lane_width=" << point.lane_width << " direction=" << point.direction << " block type=" << point.block_type << " x=" << point.x << ", y=" << point.y << ", ang=" << point.ang
            << ", k=" << point.k << ", v=" << point.v << ", s=" << point.s << ", t=" << point.t << "}";
        return out;
    }
};
