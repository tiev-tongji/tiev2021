...
public:
    GriddedPathTimeGraph(PathTimeGraph& st_data, const std::vector<Obstacle>& obstacles, const Pose& init_point, const SpeedLimit& speed_limit)
        : st_data_(st_data), obstacles_(obstacles), init_point_(init_point), dp_st_config_(), unit_s_(st_data_.path_length() / (dp_st_config_.matrix_dimension_s() - 1)),
          unit_t_(st_data_.total_time() / (dp_st_config_.matrix_dimension_t() - 1)), dp_st_cost_(dp_st_config_, obstacles, unit_t_), speed_limit_(speed_limit) {}
...
private:
     ...
     * @return Edge cost
     */
    double CalculateEdgeCost(const STPoint& first, const STPoint& second, const STPoint& third, const STPoint& fourth, const double speed_limit);
     ...
     * @return Edge cost
     */
    double CalculateEdgeCostForThirdCol(const size_t curr_row, const size_t pre_row, const double speed_limit);
};
}
