...
const double dd[5][5] = {
    2.0*sqrt2, sqrt5, 2.0, sqrt5, 2.0*sqrt2,
    sqrt5,     sqrt2, 1.0, sqrt2, sqrt5,
    2.0,       1.0,   0.0, 1.0,   2.0,
    sqrt5,     sqrt2, 1.0, sqrt2, sqrt5,
    2.0*sqrt2, sqrt5, 2.0, sqrt5, 2.0*sqrt2
};
const double lengths[] = {
    10.0 / GRID_RESOLUTION,
    5.0 / GRID_RESOLUTION,
    2.0 / GRID_RESOLUTION
};
...
const double radiuses[] = {
    5.0 / GRID_RESOLUTION,
    14.0 / GRID_RESOLUTION,
    20.0 / GRID_RESOLUTION
};
const double circle_lengths[] = {
    lengths[length_num - 1],
    lengths[length_num - 1],
    lengths[length_num - 1]
};
...
PathPlanner::PathPlanner() :
    ...
    view_controller(PathPlannerView::getInstance()),
    astar_dubins_distance_table(config->dubins_distance_table_path),
    astar_rs_distance_table(config->rs_distance_table_path){

    aStarInitializeExtendedPositions();
}

void PathPlanner::setStartMaintainedPath(const vector<Point>& start_maintained_path){
    if(is_planning) return;
	this->start_maintained_path.clear();
    this->start_maintained_path = start_maintained_path;
    for(auto& p : this->start_maintained_path) p.v = inf;
    if(!start_maintained_path.empty()) start_point = start_maintained_path.back();
	if(start_point.x < 0 || start_point.y < 0 ||
        start_point.x >= GRID_ROW || start_point.y >= GRID_COL
        || start_maintained_path.empty()){
        start_point = Point();
        start_point.x = CAR_CEN_ROW;
        start_point.y = CAR_CEN_COL;
        start_point.angle.setByRad(0);
	}
}

void PathPlanner::setTargets(const vector<Point>& targets){
    if(is_planning) return;
    this->targets = targets;
    if(targets.size() > MAX_TARGET_NUM)
        this->targets.resize(MAX_TARGET_NUM);
}
...
bool PathPlanner::getResults(vector<SpeedPath> &results){
    ...
    if(any_result){
        for(int i = 0; i < MAX_TARGET_NUM; ++i)
            if(have_result[i])
                results.emplace_back(speed_paths[i]);
    }
    return any_result;
}
...
void PathPlanner::setAbsSafeMap(int map[GRID_ROW][GRID_COL]){
    if(is_planning) return;
    memcpy(this->abs_safe_map, map, sizeof(this->abs_safe_map));
}

void PathPlanner::setLaneSafeMap(int map[GRID_ROW][GRID_COL]){
    if(is_planning) return;
    memcpy(this->lane_safe_map, map, sizeof(this->lane_safe_map));
}

void PathPlanner::setSafeMap(int map[GRID_ROW][GRID_COL]){
...
void PathPlanner::plan(){
    ...
    view_controller->setMap(safe_map);
...
void PathPlanner::run(int target_index){
    ...
    vector<Point>& res = speed_paths[target_index].path;

    if(start_maintained_path.size() > 1){
        double offset = start_maintained_path.back().s;
        res.insert( res.begin(), start_maintained_path.begin(),
            start_maintained_path.end() - 1);
        for(int i = start_maintained_path.size() - 1; i < res.size(); ++i)
            res[i].s += offset;
    }

    for(auto& p : res) p.v = velocity_limit;
...
void PathPlanner::planSpeed(int target_index){
	...
	for(int i = 1; i < speed_paths[target_index].path.size(); ++i)
		if(speed_paths[target_index].path[i].backward !=
			speed_paths[target_index].path[i - 1].backward){
			end_point = i;
			break;
		}

	vector<Point> result_tail;
	result_tail.insert(result_tail.begin(),
		speed_paths[target_index].path.begin() + end_point,
		speed_paths[target_index].path.end());
	for(auto& p : result_tail) p.v = 0;
    ...
    speed_limits[target_index].reserve(
        speed_paths[target_index].path.size()
    );

    //conversion
    for(auto& point : speed_paths[target_index].path){
        if(!point.backward) point.angle.setByRad(Angle::PI - point.angle.getRad());
        speed_limits[target_index].emplace_back(point.s,
            min(sqrt(g_tims_miu / (point.k + 0.0001)), point.v) * 0.7);
    }

	// speed_limits[target_index][0].second = current_speed;
	speed_paths[target_index].path.front().v = current_speed;
    speed_paths[target_index] = SpeedOptimizer::RunSpeedOptimizer(
        dynamic_obj_list.dynamic_obj_list,
        speed_paths[target_index].path,
        speed_limits[target_index],
        speed_paths[target_index].path.back().s
    );
	...
	speed_paths[target_index].path.insert(speed_paths[target_index].path.end(),
...
void PathPlanner::aStarPretreatment(){
    ...
    for(int i = 0; i < targets.size(); ++i)
        calculateDistanceBySPFA(safe_map, astar_distance_to_multi_targets[i].map,
            make_pair(targets[i].x, targets[i].y), astar_queues[i]);
...
double PathPlanner::aStarHeuristic(int target_index, const astate& current){
    ...
    double ta = targets[target_index].angle.getRad();
    double q0[] = {current.x, current.y, Angle::PI - current.a}, q1[] = {tx, ty, Angle::PI - ta};
    double curve_distance;
    if(backward_enabled) curve_distance = astar_rs_distance_table.getDistance(q0, q1);
    else curve_distance = astar_dubins_distance_table.getDistance(q0, q1);
    double euclidean = euclideanDistance(current.x, current.y, tx, ty);
    double p1 = max(euclidean, curve_distance);
    double p2 = aStarGetDistanceToTarget(target_index, current.x, current.y);
    double curvature_changed_punish = 0;
    if(current.prior_index >= 0)
        curvature_changed_punish = 5.0 *
        fabs(current.curvature - astar_stored_states[target_index][curvature_changed_punish].curvature);
    return (max(p1 * 0.5, p2) + curvature_changed_punish);
    #else
    return 0;
    #endif
}
...
bool PathPlanner::aStarIsTarget(int target_index, const astate& current){
    ...
    if(abs(current.a - targets[target_index].angle.getRad()) > ta) return false;
    return true;
}
...
void PathPlanner::aStarPlan(int target_index){
    ...
    while(!timeout() && !using_queue.empty()){
        ...
        double current_euclidean = euclideanDistance(current_state.x, current_state.y,
            targets[target_index].x, targets[target_index].y);
        double current_spfa = aStarGetDistanceToTarget(target_index,
        ...
        #ifndef NO_RS
        if((!backward_enabled || current_state.cost >= stop_s - 0.5) &&
            (state_cnt >= N || !analytic_expansion_first_tried)){
            ...
            if(aStarAnalyticExpansion(target_index, current_state,
                analytic_expansion_states, r)){
                if(analytic_expansion_states.size() >= 0){
                    ...
                    stored_states.insert(stored_states.end(), analytic_expansion_states.begin(),
                        analytic_expansion_states.end());
                ...
                analytic_expansion_states[0].prior_index = current_index;
                stored_states.insert(stored_states.end(),
                    analytic_expansion_states.begin(),
                    analytic_expansion_states.begin() + jump_size);
                for(int i = old_l + 1; i < old_l + jump_size; ++i){
                    stored_states[i].prior_index = i - 1;
                    if((i - old_l) % jump_step == 0){
                        using_queue.push(
                            make_pair(-aStarHeuristic(target_index, stored_states[i])
                                -stored_states[i].cost, i)
                        );
                    }
                }
            }
        ...
        aStarExtend(current_state, extended_states);

        for(auto& path : extended_states){
            if(path.size() <= 1) continue;
            bool could_reach_target = (current_euclidean <=
                (path.back().cost - path.front().cost) * 2);
            ...
            for(int i = old_l; i < stored_states.size(); ++i) stored_states[i].prior_index = i-1;
            ...
            if(aStarUpdateCost(stored_states.size() - 1, target_index))
                using_queue.push(make_pair(
                    -aStarHeuristic(target_index, stored_states.back())
                    -stored_states.back().cost, stored_states.size() - 1
                ));
        }
    ...
    if(end_index > 0){
        ...
        for(int tmp = cur; tmp >= 0; tmp = stored_states[tmp].prior_index){
            ...
            else tmp_states.push_back(state);
        ...
        std::reverse(speed_paths[target_index].path.begin(),
            speed_paths[target_index].path.end());
...
int PathPlanner::aStarAnalyticExpansionsInterval(double distance){
    return min(config->a_star_analytic_expansion_max_N,
        (int)round(exp((distance - config->a_star_analytic_expansion_param_t)
        / config->a_star_analytic_expansion_param_k)));
}
...
bool PathPlanner::aStarAnalyticExpansion(int target_index, const astate& state, vector<astate>& expansion_states, double radius){
    ...
    double q0[] = {state.x, state.y, Angle::PI - state.a};
    double q1[] = {targets[target_index].x, targets[target_index].y,
                    Angle::PI - targets[target_index].angle.getRad()};
    const double step = config->a_star_extention_step_meter / GRID_RESOLUTION;
    double q[3], last_q[3] = {state.x, state.y, Angle::PI - state.a}, x = step, length;
    if(!backward_enabled){
        DubinsPath tmpath;
        ...
        while(x < length){
            ...
            if(isCarSafeHere(q[0], q[1], Angle::PI - q[2], abs_safe_map, lane_safe_map, current_speed)){
                expansion_states.emplace_back(q[0], q[1], Angle::PI - q[2]);
                expansion_states.back().curvature = ((q[2] == last_q[2]) ? 0 : (1.0 / radius));
                expansion_states.back().cost = state.cost + x;
                expansion_states.back().backward = (
                    ((q[0] - last_q[0]) * cos(q[2]) +
                    (q[1] - last_q[1]) * sin(q[2])) < 0
                );
...
void PathPlanner::aStarExtend(const astate& source, vector<vector<astate>>& destination){
    destination.clear();

    vector<vector<astate>>* primitives_list[] = {
        &astar_speed_primitives[current_speed_id].forward_primitives,
        &astar_speed_primitives[current_speed_id].backward_primitives
    };
    const int plist_l = sizeof(primitives_list) / sizeof(void*);

    const double sina = sin(-source.a);
    const double cosa = cos(-source.a);
    const double ppa = Angle::PI + source.a;
    const double pix2 = Angle::PI * 2.0;
    //don't change direction before stop_s
    if(source.cost < stop_s - 0.5){
        if(start_point.backward) primitives_list[0] = NULL;
        else primitives_list[1] = NULL;
    }
    ...
    //
    int sfar = min(abs_safe_map[(int)round(source.x)][(int)round(source.y)],
        lane_safe_map[(int)round(source.x)][(int)round(source.y)]);
    ...
    for(int primitive_i = 0; primitive_i < plist_l; ++primitive_i){
        ...
        for(int i = start_i; i < destination.size(); ++i){
            ...
            double safe_r = 1.42 * arc_length + 0.71;
            ...
            for(auto& p : destination[i]){
                p.a = ppa - p.a;
                //TODO: maybe not essential
                while(p.a < -Angle::PI) p.a += pix2;
                while(p.a >= Angle::PI) p.a -= pix2;
...
bool PathPlanner::aStarUpdateCost(int state_index, int target_index){
    ...
    const int angle_id = (int)round((state.a + Angle::PI) / UNIT_DISCRETE_ANGLE);
...
bool PathPlanner::aStarIsLineSafe(const astate& a, const astate& b){
    ...
    for(int i = 0; i <= step_num; ++i){
        if(!isCarSafeHere(tmp.x, tmp.y, tmp.a, abs_safe_map, lane_safe_map, current_speed))
            return false;
        tmp.x += dx;
        tmp.y += dy;
    }
...
double PathPlanner::aStarGetDistanceToTarget(int target_index, double x, double y){
    ...
    for(int i = 0; i < 2; ++i)
        for(int j = 0; j < 2; ++j){
            double dis = euclideanDistance(x, y, xs[i], ys[j]) +
                astar_distance_to_multi_targets[target_index].map[xs[i]][ys[j]];
...
void PathPlanner::primitives::addPrimitive(vector<astate>& forward_primitive, bool need_mirror){
    ...
    if(need_mirror){
        ...
        backward_primitives.push_back(backward_primitives.back());
        for(auto& p : backward_primitives.back()){
            p.y = -p.y;
            p.a = -p.a;
        }
...
void PathPlanner::aStarInitializeExtendedPositions(){
    ...
    for(int spdid = 0; spdid < speed_cnt; ++spdid){
        ...
        int another_arc = (spdid + speed_cnt) / 2;
        if(another_arc != spdid) astar_speed_primitives[spdid].addPrimitive(lines[another_arc], false);
        astar_speed_primitives[spdid].addPrimitive(lines[spdid], false);

        if(another_arc != spdid) astar_speed_primitives[spdid].addPrimitive(arcs[another_arc]);
        astar_speed_primitives[spdid].addPrimitive(arcs[spdid]);
    }
}
...
void PathPlanner::generateLeftArc(int speed_id, vector<astate>& positions){
    ...
    for(double cost = 0; cost <= l; cost += step, alpha += theta){
        ...
        position.x = (cosa * ax - sina * ay + center_x) / GRID_RESOLUTION;
        position.y = (sina * ax + cosa * ay + center_y) / GRID_RESOLUTION;
        position.cost = cost / GRID_RESOLUTION;
        position.a = Angle::PI + alpha;
...
//Tool methods
void PathPlanner::calculateDistanceBySPFA(
    ...
    while(!using_queue.empty()){
        ...
        for(int i = 0; i < ldx; ++i)
            for(int j = 0; j < ldy; ++j){
                ...
                if(tx < 0 || ty < 0 || tx >= GRID_ROW || ty >= GRID_COL
                    || map[tx][ty] <= COLLISION_CIRCLE_SMALL_R / GRID_RESOLUTION)
                    continue;
