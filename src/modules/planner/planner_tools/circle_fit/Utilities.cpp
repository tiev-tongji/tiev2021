...
void SimulateArc(FitData& data, reals a, reals b, reals R, reals theta1, reals theta2, reals sigma)
/*    
...
//			isotropic Gaussian noise
		...
		data.points[i].first = a + R*cos(theta) + sigma*dx;
		data.points[i].second = b + R*sin(theta) + sigma*dy;
	}
}

//********************* SimulateRandom ****************************

void SimulateRandom(FitData& data, reals Window)
/*    
...
*/
{
	...
	{
		data.points[i].first = Window*(Two*rand()/RAND_MAX - One);
		data.points[i].second = Window*(Two*rand()/RAND_MAX - One);
	}
}
...
reals Sigma (FitData& data, FitCircle& circle)
{
    reals sum=0.,dx,dy;

    for (int i=0; i<data.n; i++)
    {
        dx = data.points[i].first - circle.a;
        dy = data.points[i].second - circle.b;
...
reals SigmaReduced (FitData& data, FitCircle& circle)
{
    int i,n=data.n;
...
reals SigmaReducedNearLinearCase (FitData& data, FitCircle& circle)
{
    int i,n=data.n;
	...
	for (W=Z=0.,i=0; i<n; i++)
	{
		x = data.points[i].first - data.meanX;
		y = data.points[i].second - data.meanY;
		z = x*x + y*y;
		p = x*c + y*s;
		t = del*z - Two*p;
...
reals SigmaReducedForCenteredScaled (FitData& data, FitCircle& circle)
{
    int i,n=data.n;
    reals sum=0.,dx,dy,r;
...
reals OptimalRadius (FitData& data, FitCircle& circle)
{
    reals Mr=0.,dx,dy;
