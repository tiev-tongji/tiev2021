...
void DubinsTableHelper::generate_distance_table_file(int r, double step, int angle_num,
    double turning_radius, string file_name){
    long long stime = time(0);
    fstream output(file_name, ios::out | ios::binary);
    if(!output.is_open() || angle_num <= 0 ||
        r <= 0 || step <= 0 ||
        turning_radius <= 0){
