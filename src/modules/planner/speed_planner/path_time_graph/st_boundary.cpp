...
STBoundary::STBoundary(STPoint blp, STPoint brp,
                       STPoint ulp, STPoint urp) {
...
bool STBoundary::GetUnblockSRange(const double curr_time, const double total_path_length,
    ...
    *s_lower = 0.0;
    ...
    double upper_cross_s = upper_left_point_.s() + r_upper *
        (upper_right_point_.s() - upper_left_point_.s());

    const double r_lower = (curr_time - bottom_left_point_.t()) /
        (bottom_right_point_.t() - bottom_left_point_.t());

    double lower_cross_s = bottom_left_point_.s() + r_lower * 
        (bottom_right_point_.s() - bottom_left_point_.s());
