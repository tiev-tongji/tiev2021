...
public:
    ...
    DpStCost(const DpStConfig& dp_st_config, const std::vector<Obstacle>& obstacles, double unit_t)
            : dp_st_config_(dp_st_config),
 ...
 */
    double GetJerkCostByTwoPoints(const double pre_speed, const double pre_acc,
                                  const STPoint& pre_point, const STPoint& curr_point);
