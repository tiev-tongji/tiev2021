...
bool GriddedPathTimeGraph::InitCostTable() {
    ...
    cost_table_ = std::vector<std::vector<StGraphPoint> >(dim_t,
            std::vector<StGraphPoint>(dim_s, StGraphPoint()));
...
double GriddedPathTimeGraph::CalculateEdgeCostForThirdCol(const size_t curr_row, const size_t pre_row,
                                                          const double speed_limit) {
    double init_speed = init_point_.v;
...
void GriddedPathTimeGraph::CalculateCostAt(size_t c, size_t r) {
    ...
    const auto max_s_diff = static_cast<size_t>(dp_st_config_.vehicle_upper_speed_limit() *
                                                 (1 + SpeedRangeBuffer) *
                                                 unit_t_ / unit_s_);
    ...
    for (size_t r_pre = r_low; r_pre <= r; ++r_pre) {
        if (pre_col[r_pre].total_cost() == std::numeric_limits<double>::infinity() ||
...
bool GriddedPathTimeGraph::RetrieveSpeedProfile(SpeedData* speed_data) {
    ...
    if (speed_profile.front().t() > Eps ||
        speed_profile.front().s() > Eps) {
