...
namespace TiEV {
QpSpeedOptimizer::QpSpeedOptimizer(const Point& init_point, const Point& target_point,
...
void QpSpeedOptimizer::Init() {
    ...
    double curr_t = 0.0;
    uint32_t num_spline =
        qp_st_config_.number_of_discrete_graph_t() - 1;
    ...
    t_evaluated_resolution_ =
            qp_st_config_.total_time() / (num_evaluated_t - 1);
...
bool QpSpeedOptimizer::Process(PathTimeGraph& st_graph_data,
	...
	std::pair<double, double> accel_bound =
		std::make_pair(qp_st_config_.max_acceleration(), qp_st_config_.max_deceleration());
	...
	*/
    ...
    for (const double curr_t : t_evaluated_) {
        double lower_s = 0.0;
        double upper_s = 0.0;
        GetSConstraintByTime(st_graph_data.st_boundaries(), curr_t, qp_st_config_.total_path_length(),
                             &upper_s, &lower_s);
    ...
    // Debug
    std::cout << "QP distance constraint: " << std::endl;
    std::cout << "t_evaluated_: " << std::endl;
    for (auto t : t_evaluated_) std::cout << t << ' ';
    std::cout << std::endl;
    std::cout << "s_lower_bound: " << std::endl;
    for (auto sl : s_lower_bound) std::cout << sl << ' ';
    std::cout << std::endl;
    std::cout << "s_upper_bound" << std::endl;
    for (auto su : s_upper_bound) std::cout << su << ' ';
    std::cout << std::endl << std::endl;

    /**
     * Add speed constraint
     */
    // TODO(Charles Peng): Apply SpeedLimit
    ...
    // Debug
    std::cout << "QP speed constarint" << std::endl;
    for (size_t i = 0; i < t_evaluated_.size(); ++i) {
        std::cout << "time = " <<  t_evaluated_[i] << " v = " << speed_upper_bound[i] << "m/s" << std::endl;
    }
    std::cout << std::endl;
	...
	*/
    ...
    if (!spline_constraint->AddThirdDerivativeSmoothConstraint()) {
        std:cout << "QpSpeedOptimizer: Fail to apply third derivative smooth constraint!" << std::endl;
        return false;
    }
   ...
   if (target_point_.s - reference_dp_speed_points.back().s() <= 1) {
       if (!spline_constraint->AddPointDerivativeConstraint(reference_dp_speed_points.back().t(), 0.0)) {
   ...
   */
   ...
   // spline_constraint->AddPointSecondDerivativeConstraint(0.0, init_point_.a);
    ...
    // Debug
    std::cout << "QP reference dp speed points" << std::endl;
    std::cout << "x_guide: ";
    for (const auto& t : t_knots_) std::cout << t << ' ';
    std::cout << std::endl;
    std::cout << "fx_guide: ";
    for (const auto& x : fx_guide) std::cout << x << ' ';
    std::cout << std::endl;
    std::cout << std::endl;
...
bool QpSpeedOptimizer::GetSConstraintByTime(const std::vector<STBoundary> boundaries,
                                            const double time,
