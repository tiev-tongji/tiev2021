...
public:
     ...
     */
    static SpeedPath RunSpeedOptimizer(const std::vector<DynamicObj>& obstacle_list,
    ...
    // Getter
    const SpeedData& dp_speed_data() { return dp_speed_data_; }

	// Getter
	const PathTimeGraph& st_data() { return st_data_; }

	// QP result getter
	const Spline1d& splines() { return qp_speed_optimizer_.splines(); }
