...
u_int32_t PointPosConverter(
    double theta_) {  // Convert the counterclockwise angle with the x-axis to
                      // the clockwise angle with the y-axis.


  //std::cout <<" converter theta_ " << theta_ << std::endl;
  while (theta_ > M_PI) {
    theta_ -= 2 * M_PI;
  }
  while (theta_ < -1 * M_PI) {
...
void CreatePathPlanningPoint(cn::seisys::v2x::pb::PathPlanningPoint &pppoint,
                             DataPoint &point, int time) {
  try {
...
int main(int argc, char *argv[]) {
  ...
  std::ifstream          inputFile("../traj_test4");
  std::vector<DataPoint> dataPoints;
  if (inputFile.is_open()) {
    std::cout << " open is sud " << std::endl;
    std::string line;
    while (std::getline(inputFile, line)) {
      DataPoint dataPoint;
      int       id;
      double    Lon, Lat, utmX, utmY, heading, curvature, mode;
      double    SpeedMode, EventMode, OppositeSideMode, LaneNum, LaneSeq,
          LaneWidth;
      std::sscanf(
          line.c_str(), "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",&id, &Lon,
          &Lat, &utmX, &utmY, &heading, &curvature, &mode, &SpeedMode,
          &EventMode, &OppositeSideMode, &LaneNum, &LaneSeq, &LaneWidth);
      double speed = 10;

      dataPoint.utmX = utmX;
      dataPoint.utmY = utmY;  
      dataPoint.heading = heading; 
      dataPoint.velocity = speed; 
      dataPoint.acceleration = 0;
      std::cout << " find for " << utmX << "," << utmY << " for head " << heading << std::endl;
  ...
  try {
    ...
    while (1) {
      ...
      cloudToV.set_sceneid(
          cn::seisys::v2x::pb::CloudToV_SceneId::CloudToV_SceneId_SCENE_ID_1);
      
      std::cout <<" siz of datapoints " << dataPoints.size() << std::endl;
      for (int i = 0; i < dataPoints.size(); i++) {
        auto *path_planning = cloudToV.add_pathplanning();
        CreatePathPlanningPoint(
            *path_planning, dataPoints[i],
            dataPoints[i].timeStamp - dataPoints[0].timeStamp);
      }
