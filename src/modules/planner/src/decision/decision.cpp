...
// #define routing
    ...
    while(true) {
        ...
        if(mm->machine.isActive<UTurn>())
            msgm->addTextInfo("FSM State", "UTurn");
        mapm->visualization();
    }
}
    ...
    while(true) {
        ...
        if(road_mode == HDMapMode::INTERSECTION_SOLID
           ...
           || road_mode == HDMapMode::PARKING) {
            max_speed =
                min(max_speed, mapm->getSpeedBySpeedMode(HDMapSpeed::LOW));
        }
        if(mm->machine.isActive<SafeDriving>()
...
void requestGlobalPathFromMapServer() {
    ...
    while(!Config::getInstance()->enable_routing_by_file) {
        ...
        if(task_list.size() > 1)
            cost =
                routing->findReferenceRoad(tmp_global_path, task_list, false);
