...
ReferenceLineInfo::ReferenceLineInfo(std::vector<HDMapPoint> reference_line,
  ...
  double learning_rate(0.24915), max_iteration(100), weight_smooth(1),
      weight_curvature(1), weight_obstacle(0);
  PathSmoother ps(learning_rate, max_iteration, weight_smooth, weight_curvature,
                  weight_obstacle);
  int          num_interpolate_points = 0;
...
double ReferenceLineInfo::GetSpeedLimitFromS(double s) {
  ...
  for (const auto& p : reference_line_) {
    if (p.s >= s) {
      ...
      return map_manager.getSpeedBySpeedMode(p.speed_mode) / GRID_RESOLUTION;
      return 0;
      // return MapManager::getInstance()->getSpeedBySpeedMode(p.speed_mode) /
      //        GRID_RESOLUTION;
    }
  }
...
double ReferenceLineInfo::GetLaneWidth() const {
  double lane_width = FLAGS_default_reference_line_width;
  if (!reference_line_.empty()) {
    lane_width = reference_line_.front().lane_width;
  }
  return lane_width;
}
