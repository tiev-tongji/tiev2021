    ...
    while(true) {
        ...
        if(current_time - global_path_update_time > interval) {
            ...
            int cost = routing->findReferenceRoad(
                tmp_global_path, map.current_task_points, blocked);
            task_points_mutex.unlock_shared();
...
vector<Pose> MapManager::getUTurnTargets() {
    ...
    for(int i = map.forward_ref_path.size() - 1; i >= 0; --i) {
        ...
        if(map.accessible_map[int(point.x)][int(point.y)]) {
            maintained_uturn_target.push_back(
                Pose(point.x, point.y, point.ang));
            break;
        }
    }
...
void MapManager::updateRefPath(bool need_opposite) {
    ...
    int search_end   = min(int(global_path.size() - 1),
                         global_path_nearest_idx + search_depth);
    global_path_nearest_idx =
        getGlobalPathNearestIndex(search_begin, search_end);
    search_begin  = max(global_path_nearest_idx - search_history_depth, 0);
    search_end    = min(int(global_path.size() - 1),
                     global_path_nearest_idx + search_depth);
    ...
    if(map.ref_path.empty())
        ...
        for(int i = 1; i < map.ref_path.size(); ++i) {
            double delta_s =
                hypot(fabs(map.ref_path[i].x - map.ref_path[i - 1].x),
...
void MapManager::addPedestrian(DynamicObjList&           dynamic_obj_list,
    ...
    for(const auto& obj : dynamic_obj_list.dynamic_obj_list) {
        ...
        int init_shortest_point_index =
            shortestPointIndex(obj_init_pose, ref_path);
    ...
    if(lane_line_block_type == LaneLineBlockType::NO_BLOCK) {
                ...
                if(p.lane_num < 2)
                    ...
                    if(p.direction == RoadDirection::RIGHT
                       || p.direction == RoadDirection::STRAIGHT)
                        ...
                        base_dis =
                            (p.lane_num - p.lane_seq - 1.5) * p.lane_width;
                    ...
                    for(int lane_i = start_lane_i; lane_i <= end_lane_i;
                        ++lane_i) {
                        base_dis = (lane_i - p.lane_seq - 0.5) * p.lane_width;
                        int k    = p.lane_width / 0.5;
                        for(int i = 1; i < k; ++i) {
                            Pose block_p = p.getLateralPose(base_dis + i * 0.5);
                            map.line_block_map[int(block_p.x)][int(block_p.y)] =
                                1;
                        }
                    }
                    if(p.direction == RoadDirection::STRAIGHT) {
                        double right_base_dis =
                            (1 - p.lane_seq - 0.5) * p.lane_width;
        ...
        if(carInRoad()) {
            ...
            if(lane_line_block_type == LaneLineBlockType::SEMI_BLOCK) {
                for(const auto& line : map.lane_line_list) {
                    for(int i = int(line.size() * 2 / 3); i < line.size();
                        ++i) {
                        auto& p = line[i];
                        if(p.type == LineType::SOLID)
                            map.line_block_map[int(p.x)][int(p.y)] = 1;
    ...
    for(int r = 0; r < MAX_ROW; ++r) {
        for(int c = 0; c < MAX_COL; ++c) {
            if(map.rain_signal.detected && map.rain_signal.signal) {
                ...
                map.static_lidar_map[r][c] =
                    (unsigned char)map.lidar.map[r][c] & 01;
    ...
    while(!obj_que.empty()) {
        ...
        for(int i = 0; i < 8; ++i) {
            ...
            if(Point2d(tx, ty).in_map()
               && map.lidar_dis_map[tx][ty]
                      > map.lidar_dis_map[x][y] + dis[i]) {
...
void MapManager::getPlanningDisMap(bool history) {
    ...
    for(int r = 0; r < MAX_ROW; ++r) {
        for(int c = 0; c < MAX_COL; ++c) {
            ...
            else if(map.line_block_map[r][c] != 0
                    || map.lidar_map[r][c] & 0x1) {
                map.planning_dis_map[r][c] = 0;
                obj_que.push(make_pair(r, c));
            }
        }
    ...
    while(!obj_que.empty()) {
        ...
        for(int i = 0; i < 8; ++i) {
            const int tx = x + dx[i], ty = y + dy[i];
            if(Point2d(tx, ty).in_map()
               && map.planning_dis_map[tx][ty]
                      > map.planning_dis_map[x][y] + dis[i]) {
                map.planning_dis_map[tx][ty] =
                    map.planning_dis_map[x][y] + dis[i];
                obj_que.push(make_pair(tx, ty));
            }
    ...
    if(!map.v_line_list.empty()) {
        ...
        if(map.v_line_list.back().front().type == LineType::BOUNDARY) {
            ...
            double most_left_dis =
                first_p.lane_width
                * (first_p.lane_num - first_p.lane_seq + 0.5);
        ...
        //其次以右边界矫正
            ...
            //计算参考路所指最右侧车道线的
            double most_right_dis =
                first_p.lane_width * (first_p.lane_seq - 1.5);
        ...
        if(ref_offset_dis != 0) {
            for(auto& p : map.ref_path) {
                //垂直偏移矫正-----------
                Point2d offset_vec =
                    offsetPoint(p.getDirectionVec(), ref_offset_dis);
                p.x += offset_vec.x;
                p.y += offset_vec.y;
                //暂时不考虑曲率变化
                //只横向矫正--------------
...
void MapManager::getLaneLineList() {
    ...
    for(const auto& p : map.forward_ref_path) {
        ...
        for(int i = 0; i < lane_num + 1; ++i) {
            Pose line_p =
                p.getLateralPose(p.lane_width * (i - p.lane_seq + 0.5));
            ...
            if(line_p.in_map() && p.mode != HDMapMode::CHANGE)
                map.lane_line_list[i].emplace_back(line_p.x, line_p.y,
                                                   line_type);
        }
        if(!lane_num_change) {
            //从右向左第i条地图车道中心线
    ...
    if(lane_num_change) {
        ...
        for(int i = map.forward_ref_path.size() - 1; i >= 0; --i) {
            ...
            for(int j = 0; j < lane_num; ++j) {
                Pose line_p =
                    p.getLateralPose(p.lane_width * (j - p.lane_seq + 1));
                if(line_p.in_map() && p.mode != HDMapMode::CHANGE)
...
void MapManager::laneMatch() {
    ...
    if(map.lane_line_list.size() == map.v_line_list.size()) {
        for(int i = 0; i < map.lane_line_list.size(); ++i) {
            int       shortest_index = shortestPointIndex(map.nav_info.car_pose,
                                                    map.lane_line_list[i]);
            ...
            double    offset =
                (vec_b.cross(vec_a) / vec_a.len()) * GRID_RESOLUTION;
            Point2d off_p;
            for(int j = 0; j < map.lane_line_list[i].size() - 1; ++j) {
                Point2d vec_d =
                    map.lane_line_list[i][j + 1] - map.lane_line_list[i][j];
...
void MapManager::maintainParkingSpots() {
    for(const auto& spot : map.parking_lot_list.parking_lot_list) {
        Point2d center_point = (spot.left_back + spot.left_front
                                + spot.right_back + spot.right_front)
                               * 0.25;
...
void MapManager::getBoundaryLine() {
    ...
    for(int i = 0; i < map.forward_ref_path.size(); ++i) {
        if(!lane_num_change
           && map.forward_ref_path[i].lane_num
    ...
    for(const auto& p : map.ref_path) {
        ...
        Pose right_line_p =
            p.getLateralPose(p.lane_width * (-p.lane_seq + 0.5));
    ...
    for(const auto& p : map.lane_line_list.front())
    ...
    if(lane_num_change) {
        for(int j = lane_line_change_id; j < map.forward_ref_path.size(); ++j) {
            ...
            if(right_line_p.in_map())
                right_boundary.emplace_back(right_line_p.x, right_line_p.y,
                                            LineType::BOUNDARY);
            Pose left_line_p = p.getLateralPose(
                p.lane_width * (p.lane_num - p.lane_seq + 0.5));
            if(left_line_p.in_map())
                left_boundary.emplace_back(left_line_p.x, left_line_p.y,
                                           LineType::BOUNDARY);
        }
...
void MapManager::getSpeedMaintainedPath(NavInfo& nav_info) {
    ...
    for(const auto& p : maintained_path) {
        ...
        speed_limits.emplace_back(
            p.s, min(max_speed, max_velocity_for_curvature(p.k)));
    }
        ...
        if(obstacle.path.size() > 1) {
            ...
            if(v / GRID_RESOLUTION < 2) {
                for(int i = 0; i < obstacle.corners.size(); ++i) {
                    ...
                    const auto& end =
                        obstacle
                            .corners[(i + 1) % (int)obstacle.corners.size()];
