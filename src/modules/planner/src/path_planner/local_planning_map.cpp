...
bool PathPlanner::local_planning_map::is_in_map(int row_idx,
                                                int col_idx) const {
    return row_idx >= 0 && row_idx < MAX_ROW && col_idx >= 0
           && col_idx < MAX_COL;
}
        ...
        for(int j = 0; j < MAX_COL; ++j) {
            ...
            for(int a = 0; a < XYA_MAP_DEPTH; ++a) {
                if(a_vec[a] < output_map[i][j].first) {
                    ...
                    output_map[i][j].second =
                        M_PI - (a + 0.5) * XYA_MAP_DELTA_A;
                }
            }
        }
}
...
int PathPlanner::local_planning_map::get_angle_index(double ang) {
    return (int)(wrap_angle_0_2_PI(M_PI - ang) / XYA_MAP_DELTA_A)
           % XYA_MAP_DEPTH;
}
...
#define vec(a, b)                                             \
    ...
    constexpr pair<int, double> deltas[] = { vec(1, 0),  vec(1, -1),
                                             vec(0, -1), vec(-1, -1),
                                             vec(-1, 0), vec(-1, 1),
                                             vec(0, 1),  vec(1, 1) };
#undef vec
...
#define UPDATE(_zw0_)                                                       \
    MULTILINE(if(_zw0_ >= 0 && flatten_safe_map[_zw0_ / XYA_MAP_DEPTH]) {   \
        for(int da = -1; da <= 1; ++da) {                                   \
            double turning_punishment = deltas[delta_idx].second * fabs(da) \
                                        * TURNING_PUNISHMENT_FACTOR;        \
...
double
PathPlanner::local_planning_map::get_maximum_safe_distance(int row_idx,
                                                           int col_idx) const {
