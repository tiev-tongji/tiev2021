...
void PathPlanner::runPlanner(
    ...
    double max_speed, bool reverse, const double abs_safe_map[MAX_ROW][MAX_COL],
    const double        lane_safe_map[MAX_ROW][MAX_COL],
    const vector<Pose>& start_maintained_path, const vector<Pose>& targets,
    const double current_speed, vector<SpeedPath>& results) {
...
void PathPlanner::setStartMaintainedPath(
  ...
  if (!start_point.in_map() || this->start_maintained_path.empty()) {
...
bool PathPlanner::getResults(vector<SpeedPath>& results) {
  ...
  for (int i = 0; i < MAX_TARGET_NUM; ++i) any_result |= have_result[i];
  if (any_result) {
...
void PathPlanner::planner_thread(int target_index) {
  ...
  astate start_state  = {start_point.x, start_point.y, start_point.ang,
         ...
         target_state = {
             ...
             targets[target_index].ang, 0.0,
             targets[target_index].k,   targets[target_index].v < 0.0};

  hybrid_astar_planner* planner = hybrid_astar_planners + target_index;

  const base_primitive_set* primitive_set = &clothoid_base_primitives;

  log_0("thread ", target_index, " planner ready");
  planner->plan(ref_path, start_state, target_state, current_speed,
                backward_enabled, abs_safe_map, lane_safe_map,
                config->plan_time_limit_ms * 1000, primitive_set);
  ...
  if (planner->get_have_result()) {
    ...
    speed_paths[target_index].path.reserve(result.size() +
                                           start_maintained_path.size());
    speed_paths[target_index].path.assign(start_maintained_path.begin(),
    ...
    log_0("thread ", target_index, " no result, abort");
}
...
void PathPlanner::planSpeed(int target_index) {
  ...
  speed_paths[target_index]                = SpeedOptimizer::RunSpeedOptimizer(
      dynamic_obj_list.dynamic_obj_list, speed_paths[target_index].path,
      speed_limits[target_index], speed_paths[target_index].path.back().s);
