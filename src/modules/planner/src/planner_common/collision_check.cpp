...
bool collision(double x, double y, double ang,
               const double dis_map[MAX_ROW][MAX_COL], double expansion_r) {
...
bool collision(const vector<Pose>& path, const double dis_map[MAX_ROW][MAX_COL],
               double expansion_r) {
...
bool collision(double x, double y, const double dis_map[MAX_ROW][MAX_COL],
  ...
  int x_ = round(x);
  int y_ = round(y);
  if (x_ < 0 || x_ >= MAX_ROW || y_ < 0 || y_ >= MAX_COL) return false;
  if (dis_map[x_][y_] <=
      (COLLISION_CIRCLE_SMALL_R + expansion_r) / GRID_RESOLUTION)
    return true;
  return false;
}
