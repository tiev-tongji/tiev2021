...
Pose PathMatcher::MatchToPath(const std::vector<Pose>& path, const double x, const double y) {
...
Pose PathMatcher::FindProjectionPoint(const Pose& p0, const Pose& p1, const double x, const double y) {
...
std::pair<double, double> PathMatcher::GetPathFrenetCoordinate(const std::vector<Pose>& path, const double x, const double y) {
    Pose   matched_path_point = MatchToPath(path, x, y);
