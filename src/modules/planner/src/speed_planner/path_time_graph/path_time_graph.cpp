...
#include "path_time_graph/st_point.h"

namespace TiEV {
PathTimeGraph::PathTimeGraph(std::vector<Obstacle>& obstacles, const std::vector<Pose>& path, const double s_start, const double s_end, const double t_start, const double t_end) {
...
Pose PathTimeGraph::GetPointAtTime(const Obstacle& obstacle, double time) const {
    ...
    return InterpolateUsingLinearApproximationWithT(*(it_lower - 1), *it_lower, time);
}

Box PathTimeGraph::GetStaticBoundingBox(const Obstacle& obstacle) {
    return Box(Vec(obstacle.path.front().x, obstacle.path.front().y), obstacle.length, obstacle.width, obstacle.path.front().ang);
}

Box PathTimeGraph::GetDynamicBoundingBox(const Obstacle& obstacle, const Pose& point) {
    return Box(Vec(point.x, point.y), obstacle.length, obstacle.width, point.ang);
}

SLBoundary PathTimeGraph::ComputeObstacleSLBoundary(const std::vector<Vec>& vertices, const std::vector<Pose>& path) {
...
void PathTimeGraph::SetupObstacles(std::vector<Obstacle>& obstacles, const std::vector<Pose>& path) {
...
void PathTimeGraph::SetDynamicObstacle(Obstacle& obstacle, const std::vector<Pose>& path) {
