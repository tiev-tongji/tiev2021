...
bool STBoundary::IsPointInBoundary(const STPoint& st_point) const {
    ...
    Vec upper_left_vec   = Vec(upper_left_point_.t() - st_point.t(), upper_left_point_.s() - st_point.s());
    Vec upper_right_vec  = Vec(upper_right_point_.t() - st_point.t(), upper_right_point_.s() - st_point.s());
    Vec bottom_left_vec  = Vec(bottom_left_point_.t() - st_point.t(), bottom_left_point_.s() - st_point.s());
    Vec bottom_right_vec = Vec(bottom_right_point_.t() - st_point.t(), bottom_right_point_.s() - st_point.s());

    /**
     * If st_point is in the STBoundary, the upper-cross-product
     * and the bottom-cross-product should be signed differnetly
     */
    double check_upper  = upper_left_vec.x() * upper_right_vec.y() - upper_left_vec.y() * upper_right_vec.x();
    double check_bottom = bottom_left_vec.x() * bottom_right_vec.y() - bottom_left_vec.y() * bottom_right_vec.x();
...
bool STBoundary::GetUnblockSRange(const double curr_time, const double total_path_length, double* s_upper, double* s_lower) const {
    ...
    *s_lower = 0.0;
    ...
    const double r_upper = (curr_time - upper_left_point_.t()) / (upper_right_point_.t() - upper_left_point_.t());

    double upper_cross_s = upper_left_point_.s() + r_upper * (upper_right_point_.s() - upper_left_point_.s());

    const double r_lower = (curr_time - bottom_left_point_.t()) / (bottom_right_point_.t() - bottom_left_point_.t());

    double lower_cross_s = bottom_left_point_.s() + r_lower * (bottom_right_point_.s() - bottom_left_point_.s());
