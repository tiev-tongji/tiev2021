...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  for (const auto& obstacle : obstacles_) {
    ...
    double r1 =
        (t - boundary.bottom_left_point().t()) /
        (boundary.bottom_right_point().t() - boundary.bottom_left_point().t());
    s_lower = boundary.bottom_left_point().s() +
              r1 * (boundary.bottom_right_point().s() -
                    boundary.bottom_left_point().s());
    double r2 =
        (t - boundary.upper_left_point().t()) /
        (boundary.upper_right_point().t() - boundary.upper_left_point().t());
    s_upper = boundary.upper_left_point().s() +
              r2 * (boundary.upper_right_point().s() -
                    boundary.upper_left_point().s());

    if (s < s_lower) {
      const double follow_distance =
          dp_st_config_.safe_distance() +
          obstacle.path.front().v * dp_st_config_.safe_time_buffer();
      // const double follow_distance =  obstacle.path.front().v *
      // dp_st_config_.safe_time_buffer();
...
double DpStCost::GetSpeedCost(const STPoint& first, const STPoint& second,
  ...
  if (det_speed > 0) {
    cost += dp_st_config_.exceed_speed_penalty() * (det_speed * det_speed) *
            unit_t_;
  } else if (det_speed < 0) {
    cost +=
        dp_st_config_.low_speed_penalty() * (det_speed * det_speed) * unit_t_;
  }
