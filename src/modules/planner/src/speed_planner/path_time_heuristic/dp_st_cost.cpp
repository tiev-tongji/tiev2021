#include "path_time_heuristic/dp_st_cost.h"
#include <cmath>
#include <iostream>
...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
    ...
    for(const auto& obstacle : obstacles_) {
        ...
        double r1 = (t - boundary.bottom_left_point().t()) / (boundary.bottom_right_point().t() - boundary.bottom_left_point().t());
        s_lower   = boundary.bottom_left_point().s() + r1 * (boundary.bottom_right_point().s() - boundary.bottom_left_point().s());
        double r2 = (t - boundary.upper_left_point().t()) / (boundary.upper_right_point().t() - boundary.upper_left_point().t());
        s_upper   = boundary.upper_left_point().s() + r2 * (boundary.upper_right_point().s() - boundary.upper_left_point().s());
...
double DpStCost::GetAccelCost(const double accel) {
    ...
    cost += accel_sq * decel_penalty * decel_penalty / (1 + std::exp(1.0 * (accel - max_dec))) + accel_sq * accel_penalty * accel_penalty / (1 + std::exp(-1.0 * (accel - max_acc)));

    return cost * unit_t_;
}

double DpStCost::GetAccelCostByTwoPoints(const double pre_speed, const STPoint& first, const STPoint& second) {
...
double DpStCost::GetJerkCostByTwoPoints(const double pre_speed, const double pre_acc, const STPoint& pre_point, const STPoint& curr_point) {
...
double DpStCost::GetJerkCostByThreePoints(const double first_speed, const STPoint& first_point, const STPoint& second_point, const STPoint& third_point) {
...
double DpStCost::GetJerkCostByFourPoints(const STPoint& first, const STPoint& second, const STPoint& third, const STPoint& fourth) {
