...
bool GriddedPathTimeGraph::Search(SpeedData* speed_data) {
  ...
  for (const auto& boundary : st_data_.st_boundaries()) {
    if (boundary.IsPointInBoundary({0.0, 0.0}) ||
        (std::fabs(boundary.min_t()) < kBoundaryEpsilon &&
         std::fabs(boundary.min_s()) < kBoundaryEpsilon)) {
      std::vector<SpeedPoint> speed_profile;
      double                  t = 0.0;
      for (int i = 0; i < dp_st_config_.matrix_dimension_t();
           i++, t += unit_t_) {
...
void GriddedPathTimeGraph::GetRowRange(const StGraphPoint& point,
  ...
  const double delta_s_upper_bound =
      v0 * unit_t_ + 0.5 * dp_st_config_.max_acceleration() * speed_coeff;
  ...
  *next_highest_row =
  ...
  const double delta_s_lower_bound =
      v0 * unit_t_ + 0.5 * dp_st_config_.max_deceleration() * speed_coeff;

  int nlr = static_cast<int>(point.index_s()) +
            static_cast<int>(delta_s_lower_bound / unit_s_);
...
double GriddedPathTimeGraph::CalculateEdgeCostForSecondCol(
    const size_t row, const double speed_limit) {
  ...
  double speed_cost =
      dp_st_cost_.GetSpeedCost(pre_point, curr_point, speed_limit);
  double accel_cost =
      dp_st_cost_.GetAccelCostByTwoPoints(init_speed, pre_point, curr_point);
  double jerk_cost = dp_st_cost_.GetJerkCostByTwoPoints(init_speed, init_acc,
                                                        pre_point, curr_point);
  return speed_cost + accel_cost + jerk_cost;
}

double GriddedPathTimeGraph::CalculateEdgeCostForThirdCol(
    const size_t curr_row, const size_t pre_row, const double speed_limit) {
  double         init_speed = init_point_.v;
...
void GriddedPathTimeGraph::CalculateCostAt(size_t c, size_t r) {
  ...
  if (c == 1) {
    ...
    if (acc < dp_st_config_.max_deceleration() ||
        acc > dp_st_config_.max_acceleration()) {
      return;
    }
    ...
    double cost = cost_cr.obstacle_cost() + cost_init.total_cost() +
  ...
  const auto   max_s_diff =
      static_cast<size_t>(dp_st_config_.vehicle_upper_speed_limit() *
                          (1 + SpeedRangeBuffer) * unit_t_ / unit_s_);
  ...
  if (c == 2) {
    for (size_t r_pre = r_low; r_pre <= r; ++r_pre) {
      curr_speed_limit = std::fmin(curr_speed_limit,
                                   speed_limit_.GetSpeedLimit(unit_s_ * r_pre));
      const double acc =
      ...
      for (const auto& st_boundary : st_data_.st_boundaries()) {
        if (st_boundary.HasOverlap(cost_cr.st_point(),
                                   pre_col[r_pre].st_point())) {
          hasOverLap = true;
          break;
        }
      }
  ...
  for (size_t r_pre = r_low; r_pre <= r; ++r_pre) {
    ...
    const double curr_a = (cost_cr.index_s() * unit_s_ +
                          ...
                          (unit_t_ * unit_t_);
    if (curr_a > dp_st_config_.max_acceleration() ||
        curr_a < dp_st_config_.max_deceleration()) {
      continue;
    }
    ...
    if (prepre_graph_point.total_cost() ==
        std::numeric_limits<double>::infinity()) {
      continue;
    }
    ...
    const STPoint& triple_pre_point =
        prepre_graph_point.pre_point()->st_point();
...
bool GriddedPathTimeGraph::RetrieveSpeedProfile(SpeedData* speed_data) {
  ...
  for (size_t i = 0; i + 1 < speed_profile.size(); ++i) {
    const double v = (speed_profile[i + 1].s() - speed_profile[i].s()) /
                     (speed_profile[i + 1].t() - speed_profile[i].t());
