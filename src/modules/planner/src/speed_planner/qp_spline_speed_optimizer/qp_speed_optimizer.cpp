...
namespace TiEV {
QpSpeedOptimizer::QpSpeedOptimizer(const Pose&       init_point,
                                   const Pose&       target_point,
    ...
    : qp_st_config_(),
      ...
      splines_(),
      t_knots_resolution_(
          qp_st_config_.total_time() /
          (static_cast<int>(qp_st_config_.number_of_discrete_graph_t()) - 1)),
...
bool QpSpeedOptimizer::Process(PathTimeGraph&   st_graph_data,
                               const SpeedData& reference_dp_speed_points) {
  std::pair<double, double> accel_bound = std::make_pair(
      qp_st_config_.max_acceleration(), qp_st_config_.max_deceleration());
  ...
  for (const double curr_t : t_evaluated_) {
    double lower_s = 0.0;
    double upper_s = 0.0;
    GetSConstraintByTime(st_graph_data.st_boundaries(), curr_t,
                         qp_st_config_.total_path_length(), &upper_s, &lower_s);
    s_upper_bound.push_back(upper_s);
    s_lower_bound.push_back(lower_s);
  }
  if (!spline_constraint->AddBoundary(t_evaluated_, s_lower_bound,
                                      s_upper_bound)) {
  ...
  if (!spline_constraint->AddDerivativeBoundary(t_evaluated_, speed_lower_bound,
                                                speed_upper_bound)) {
    std::cout << "QpSpeedOptimizer: Fail to apply speed constraint!"
              << std::endl;
    return false;
  }
   ...
   */
  std::vector<double> accel_lower_bound(t_evaluated_.size(),
                                        accel_bound.second);
  std::vector<double> accel_upper_bound(t_evaluated_.size(), accel_bound.first);

  if (!spline_constraint->AddSecondDerivativeBoundary(
          t_evaluated_, accel_lower_bound, accel_upper_bound)) {
    std::cout << "QpSpeedOptimizer: Fail to apply acceleration constraint!"
              << std::endl;
    return false;
  }
  ...
  if (!spline_constraint->AddSmoothConstraint()) {
    std::cout << "QpSpeedOptimizer: Fail to apply Smooth constraint!"
              << std::endl;
    return false;
  }
  if (!spline_constraint->AddDerivativeSmoothConstraint()) {
    std::cout << "QpSpeedOptimizer: Fail to apply derivative smooth constraint!"
              << std::endl;
    return false;
  }
  if (!spline_constraint->AddSecondDerivativeSmoothConstraint()) {
    std::cout << "QpSpeedOptimizer: Fail to apply second derivative smooth "
                 "constraint!"
              << std::endl;
    return false;
  }
  ...
  if (!spline_constraint->AddMonotoneInequalityConstraintAtKnots()) {
    std::cout
        << "QpSpeedOptmizer: Fail to apply monotone inequality constarint!"
        << std::endl;
    return false;
  }
  ...
  if (!spline_constraint->AddPointConstraint(0.0, 0.0)) {
    std::cout << "QpSpeedOptimizer: Fail to apply first point constraint!"
              << std::endl;
    return false;
  }
  ...
  if (!spline_constraint->AddPointDerivativeConstraint(0.0, init_point_.v)) {
    std::cout << "QpSpeedOptimizer: Fail to apply initial speed constraint!"
              << std::endl;
    return false;
  }
...
bool QpSpeedOptimizer::GetSConstraintByTime(
  ...
  *s_upper_bound = total_path_length;
  ...
  for (auto boundary : boundaries) {
    ...
    if (!boundary.GetUnblockSRange(time, total_path_length, &s_upper,
                                   &s_lower)) {
