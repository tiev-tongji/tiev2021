#include "qp_spline_speed_optimizer/spline_1d_kernel.h"
#include "qp_spline_speed_optimizer/spline_seg_kernel.h"
#include <algorithm>
...
Spline1dKernel::Spline1dKernel(const std::vector<double>& x_knots, const uint32_t spline_order) : x_knots_(x_knots), spline_order_(spline_order) {
    total_params_  = (static_cast<uint32_t>(x_knots.size()) > 1 ? (static_cast<uint32_t>(x_knots.size()) - 1) * (1 + spline_order_) : 0);
...
bool Spline1dKernel::AddKernel(const Eigen::MatrixXd& kernel, const Eigen::MatrixXd& offset, const double weight) {
    if(kernel.rows() != kernel.cols() || kernel.rows() != kernel_matrix_.rows() || offset.cols() != 1 || offset.rows() != offset_.rows()) {
        return false;
    }
...
Eigen::MatrixXd& Spline1dKernel::kernel_matrix() {
    return kernel_matrix_;
}
...
void Spline1dKernel::AddNthDerivativekernelMatrix(const uint32_t n, const double weight) {
    SplineSegKernel::Instance()->spline_seg_kernel_mtx.lock();
    ...
    SplineSegKernel::Instance()->spline_seg_kernel_mtx.unlock();
}
...
void Spline1dKernel::AddNthDerivativekernelMatrixForSplineK(const uint32_t n, const uint32_t k, const double weight) {
    ...
    SplineSegKernel::Instance()->spline_seg_kernel_mtx.lock();
    const uint32_t  num_params = spline_order_ + 1;
    Eigen::MatrixXd cur_kernel = 2 * SplineSegKernel::Instance()->NthDerivativeKernel(n, num_params, x_knots_[k + 1] - x_knots_[k]) * weight;
...
bool Spline1dKernel::AddReferenceLineKernelMatrix(const std::vector<double>& x_coord, const std::vector<double>& ref_x, const double weight) {
