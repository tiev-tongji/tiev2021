...
SpeedData::SpeedData(std::vector<SpeedPoint> speed_points) : std::vector<SpeedPoint>(std::move(speed_points)) {
    std::sort(begin(), end(), [](const SpeedPoint& sp1, const SpeedPoint& sp2) { return sp1.t() < sp2.t(); });
}

bool SpeedData::AppendSpeedPoint(const double s, const double tt, const double v, const double a, const double da) {
...
double SpeedData::GetSByTime(double time) const {
    ...
    auto comp = [](const SpeedPoint point, const double t) { return point.t() < t; };
    ...
    return lerp((*(itr - 1)).s(), (*(itr - 1)).t(), (*itr).s(), (*itr).t(), time);
}

double SpeedData::GetVelocityByS(double s) const {
    auto comp = [](const SpeedPoint point, const double s) { return point.s() < s; };
    ...
    return lerp((*(itr - 1)).v(), (*(itr - 1)).s(), (*itr).v(), (*itr).s(), s);
}
...
double SpeedData::GetAccelByS(double s) const {
    ...
    return lerp((*(itr - 1)).a(), (*(itr - 1)).s(), (*itr).a(), (*itr).s(), s);
}
...
double SpeedData::GetTimeByS(double s) const {
    ...
    return lerp((*(itr - 1)).t(), (*(itr - 1)).s(), (*itr).t(), (*itr).s(), s);
}
