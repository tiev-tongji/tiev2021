...
SpeedOptimizer::SpeedOptimizer(std::vector<Obstacle>& obstacle_list, std::vector<Pose>& trajectory, const std::vector<std::pair<double, double>>& speed_limit, double s_start, double s_end,
...
bool SpeedOptimizer::Process(SpeedPath& speed_path) {
    ...
    if(trajectory_.back().s > 5.0 && QP_Process() && qp_speed_optimizer_.RetrieveSpeedData(spline_speed_data_)) {
        ...
        for(size_t i = 0; i < dp_speed_data_.size(); ++i) {
            points[i]   = SplineLib::Vec2f(dp_speed_data_[i].t(), dp_speed_data_[i].s());
            velocity[i] = SplineLib::Vec2f(dp_speed_data_[i].t(), dp_speed_data_[i].v());
        }
