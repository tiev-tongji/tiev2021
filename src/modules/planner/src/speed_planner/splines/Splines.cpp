...
inline Vec4f operator*(float s, Vec4f a) {
...
inline bool Larger(const Bounds2f& bb, float t) {
  Vec2f d = bb.max - bb.min;
  return d.x > t || d.y > t;
}
inline bool Larger(const Bounds3f& bb, float t) {
  Vec3f d = bb.max - bb.min;
...
template <class T>
inline int size_i(const T& container) {
...
{
  return Vec4f(t.x - 3.0f * t.y + 3.0f * t.z - t.w,
               3.0f * t.y - 6.0f * t.z + 3.0f * t.w, 3.0f * t.z - 3.0f * t.w,
               t.w);
}
...
{
  return Vec4f(b.x, -3.0f * b.x + 3.0f * b.y,
...
/// Returns bounds of the convex hull
{
  ...
  return Vec2f(Min(b01.x, b23.x), Max(b01.y, b23.y));
}
...
/// Returns accurate bounds taking extrema into account.
{
  ...
  if (cx2 < 0.0f) return bounds;  // no roots!
...
inline void Split(float t, const Vec4f& spline, Vec4f* spline0,
                  Vec4f* spline1) {
...
inline void Split(const Vec4f& spline, Vec4f* spline0, Vec4f* spline1) {
  ...
  float s0 =
      (r0 + r1) *
      0.5f;  // q0 + 2q1 + q2 / 4 = x+y + 2(y+z) + z+w / 8 = x + 3y + 3z + w
...
/// Update index for t if necessary, but don't run off array
{
  ...
  while (t > 1.0f) {
    if (index >= numSplines - 1) return false;
...
cSpline2 SplineLib::BezierSpline(const Vec2f& p0, const Vec2f& p1,
...
cSpline2 SplineLib::HermiteSpline(const Vec2f& p0, const Vec2f& p1,
...
cSpline2 SplineLib::CatmullRomSpline(const Vec2f& p0, const Vec2f& p1,
                                     const Vec2f& p2, const Vec2f& p3) {
...
void SplineLib::CircleSplines(const Vec2f& p, float r, cSpline2 splines[4]) {
  for (int i = 0; i < 4; i++) splines[i] = QuadrantSpline(p, r, i);
}

namespace {
inline cSpline2 SplineFromPoints2(const char* p8, size_t stride, int i0, int i1,
                                  int i2, int i3, float tension) {
...
int SplineLib::SplinesFromPoints(int numPoints, const Vec2f pi[],
                                 int maxSplines, cSpline2 splines[],
                                 float tension, size_t stride) {
...
int SplineLib::SplinesFromBezier(int numPoints, const Vec2f points[],
                                 const Vec2f hullPoints[], cSpline2 splines[],
                                 bool split) {
  ...
  for (int i = 0; i < numSplines; i++) {
    splines[i] =
        BezierSpline(points[0], hullPoints[0], hullPoints[1], points[1]);
    points += advance;
    hullPoints += advance;
  }
...
int SplineLib::SplinesFromHermite(int numPoints, const Vec2f points[],
                                  const Vec2f tangents[], cSpline2 splines[],
                                  bool split) {
  int numSplines = split ? numPoints / 2 : numPoints - 1;
  int advance    = split ? 2 : 1;
...
{
  return Vec2f(dot(spline.xb, w), dot(spline.yb, w));
}
}  // namespace
...
float SplineLib::LengthEstimate(const cSpline2& s, float* error) {
  // Our convex hull is p0, p1, p2, p3, so p0_p3 is our minimum possible length,
  // and p0_p1 + p1_p2 + p2_p3 our maximum.
...
float SplineLib::Length(const cSpline2& s, float t0, float t1, float maxError) {
  ...
  if (t0 == 0.0f) {
    if (t1 == 1.0f) return Length(s, maxError);
    ...
    if (t1 == 1.0f) return Length(s1, maxError);
...
void SplineLib::Split(const cSpline2& spline, cSpline2* spline0,
                      cSpline2* spline1) {
  ::Split(spline.xb, &spline0->xb, &spline1->xb);
  ::Split(spline.yb, &spline0->yb, &spline1->yb);
}

void SplineLib::Split(const cSpline2& spline, float t, cSpline2* spline0,
                      cSpline2* spline1) {
  ::Split(t, spline.xb, &spline0->xb, &spline1->xb);
  ::Split(t, spline.yb, &spline0->yb, &spline1->yb);
}

bool SplineLib::Join(const cSpline2& s0, const cSpline2& s1,
                     cSpline2* splineOut) {
  return ::Join(s0.xb, s1.xb, &splineOut->xb) &&
...
void SplineLib::Join(vector<cSpline2>* splinesIn) {
  ...
  if (prevS) splines.push_back(*prevS);

  splinesIn->swap(splines);
}

namespace {
void SubdivideForLength(const cSpline2& s, vector<cSpline2>* splines,
                        float tolerance) {
  float error;
  float length = LengthEstimate(s, &error);
...
void SplineLib::SubdivideForLength(vector<cSpline2>* splinesIn,
                                   float             tolerance) {
  vector<cSpline2> splines;
...
void SubdivideForT(const cSpline2& s, vector<cSpline2>* splines,
                   float tolerance) {
  float splitT;
...
void SplineLib::SubdivideForT(vector<cSpline2>* splinesIn, float tolerance) {
  ...
  for (const cSpline2& s : *splinesIn) ::SubdivideForT(s, &splines, tolerance);
...
inline float ClosestPoint(const Vec2f& p, const Vec2f& p0, const Vec2f& p1) {
  ...
  if (dvw <= 0.0f) return 0.0f;

  float dww = dot(w, w);

  if (dvw >= dww) return 1.0f;

  return dvw / dww;
}

void FindClosestPointNewtonRaphson(const cSpline2& spline, Vec2f p, float sIn,
                                   int maxIterations, float* tOut,
                                   float* dOut) {
  ...
  for (int i = 0; i < maxIterations; i++) {
    float ss =
        Clamp(sk, width,
              1.0f - width);  // so can interpolate points for Newtons method
    ...
    sn = Clamp(sn, sk - maxJump,
               sk + maxJump);  // avoid large steps, often unstable.
    ...
    maxJump *= 0.5f;  // reduce on a schedule -- helps binary search back
                      // towards a jump that is valid.
...
float SplineLib::FindClosestPoint(const Vec2f& p, int numSplines,
                                  const cSpline2 splines[], int* index) {
...
namespace {
void FindMinMaxDistance2s(const Vec2f& p, const Bounds2f& bbox, float* minD2,
                          float* maxD2) {
  const Vec2f& p0 = bbox.min;
  const Vec2f& p1 = bbox.max;

  // Find the nearest point to p inside the bbox
  // This can be a bbox vertex, a point on an edge or face, or p itself if it's
  // inside the box
  ...
  float maxX =
      d0.x > d1.x ? p0.x : p1.x;  // select the coordinate we're farthest from
...
void FindMinMaxDistance2s(const Vec2f& p, const cSpline2& spline, float* minD2,
                          float* maxD2) {
...
int SplineLib::FindNearbySplines(const Vec2f& p, int numSplines,
                                 const cSpline2       splines[],
                                 vector<cSubSpline2>* results,
                                 float* smallestFarOut, int numIter) {
  ...
  for (int i = 0; i < numSplines; i++) {
    ...
    if (near < smallestFar) {
      ...
      if (near < smallestNear) smallestNear = near;
      ...
      if (far < smallestFar) {
        ...
        for (int j = 0, nj = size_i(nearSplines); j < nj; j++)
          if (nearSplines[j].mD2 < smallestFar) {
            if (dj < j) nearSplines[dj] = nearSplines[j];
      ...
      if (maxSize < size_i(nearSplines)) maxSize = size_i(nearSplines);
    }
  }
  ...
  for (int i = 0; i < numIter; i++) {
    ...
    for (int i = 0; i < numNearSplines2; i++) {
      ...
      if (far < smallestFar) smallestFar = far;
      if (near < smallestNear) smallestNear = near;
    ...
    for (int i = 0; i < numNearSplines2; i++)
      if (nearSplines[i].mD2 < smallestFar) {
        if (di < i) nearSplines[di] = nearSplines[i];
  ...
  if (smallestFarOut) *smallestFarOut = smallestFar;
...
float SplineLib::FindClosestPoint(const Vec2f& p, int numSplines,
                                  const cSpline2             splines[],
                                  const vector<cSubSpline2>& nearbySplines,
                                  int*                       index) {
  int   prevParent = -1;
  float minD       = FLT_MAX;
  float minT       = 0.0f;
...
int FindSubSplineIntersections(const cSubSplineT2& spline0,
                               const cSubSplineT2& spline1, int dest,
                               int maxDest, float results[][2],
                               float tolerance) {
  ...
  if (!Intersects(bbox0, bbox1)) return dest;
  ...
  if (Larger(bbox0, tolerance)) {
    ...
    if (Larger(bbox1, tolerance)) {
      ...
      dest = FindSubSplineIntersections(spline00, spline10, dest, maxDest,
                                        results, tolerance);
      if (dest < maxDest)
        dest = FindSubSplineIntersections(spline01, spline10, dest, maxDest,
                                          results, tolerance);
      if (dest < maxDest)
        dest = FindSubSplineIntersections(spline00, spline11, dest, maxDest,
                                          results, tolerance);
      if (dest < maxDest)
        dest = FindSubSplineIntersections(spline01, spline11, dest, maxDest,
                                          results, tolerance);
    } else {
      dest = FindSubSplineIntersections(spline00, spline1, dest, maxDest,
                                        results, tolerance);
      if (dest < maxDest)
        dest = FindSubSplineIntersections(spline01, spline1, dest, maxDest,
                                          results, tolerance);
    }
  ...
  if (Larger(bbox1, tolerance)) {
    ...
    dest = FindSubSplineIntersections(spline0, spline10, dest, maxDest, results,
                                      tolerance);
    if (dest < maxDest)
      dest = FindSubSplineIntersections(spline0, spline11, dest, maxDest,
                                        results, tolerance);
...
int SplineLib::FindSplineIntersections(const cSpline2& spline0,
                                       const cSpline2& spline1, int maxResults,
                                       float results[][2], float tolerance) {
  if (maxResults <= 0) return 0;

  cSubSplineT2 subSpline0 = {spline0, 0.0f, 1.0f};
  cSubSplineT2 subSpline1 = {spline1, 0.0f, 1.0f};

  return FindSubSplineIntersections(subSpline0, subSpline1, 0, maxResults,
                                    results, tolerance);
}

namespace {
int FindSplineIntersections(int is0, int numSplines0, const cSpline2 splines0[],
                            int is1, int numSplines1, const cSpline2 splines1[],
                            int maxResults, int resultsI[][2],
                            float resultsT[][2], float tolerance) {
  if (maxResults <= 0 || numSplines0 == 0 || numSplines1 == 0) return 0;

  int count = 0;

  // once the lists are small enough, brute-force the remainder, as
  // recalculating the bounds is not free
  if (numSplines0 >= 4 || numSplines1 >= 4) {
    ...
    for (int i = 1; i < numSplines0; i++) Add(b0, FastBounds(splines0[i]));
    for (int i = 1; i < numSplines1; i++) Add(b1, FastBounds(splines1[i]));

    if (!Intersects(b0, b1)) return 0;
    ...
    count += FindSplineIntersections(
        is0 + 0, n00, splines0 + 0, is1 + 0, n10, splines1 + 0,
        maxResults - count, resultsI + count, resultsT + count, tolerance);
    count += FindSplineIntersections(
        is0 + 0, n00, splines0 + 0, is1 + n10, n11, splines1 + n10,
        maxResults - count, resultsI + count, resultsT + count, tolerance);
    count += FindSplineIntersections(
        is0 + n00, n01, splines0 + n00, is1 + 0, n10, splines1 + 0,
        maxResults - count, resultsI + count, resultsT + count, tolerance);
    count += FindSplineIntersections(
        is0 + n00, n01, splines0 + n00, is1 + n10, n11, splines1 + n10,
        maxResults - count, resultsI + count, resultsT + count, tolerance);
  ...
  for (int i0 = 0; i0 < numSplines0; i0++)
    for (int i1 = 0; i1 < numSplines1; i1++) {
      ...
      int numIntersections = FindSubSplineIntersections(
          st0, st1, 0, maxResults - count, resultsT + count, tolerance);
...
int SplineLib::FindSplineIntersections(int            numSplines0,
                                       const cSpline2 splines0[],
                                       int            numSplines1,
                                       const cSpline2 splines1[],
                                       int maxResults, int resultsI[][2],
                                       float resultsT[][2], float tolerance) {
  return ::FindSplineIntersections(0, numSplines0, splines0, 0, numSplines1,
                                   splines1, maxResults, resultsI, resultsT,
                                   tolerance);
}

int SplineLib::FindSplineIntersections(int numSplines, const cSpline2 splines[],
                                       int maxResults, int resultsI[][2],
                                       float resultsT[][2], float tolerance)
// Find self-intersections
{
  if (maxResults <= 0 || numSplines == 0) return 0;
  ...
  if (numSplines >= 8) {
    ...
    count += ::FindSplineIntersections(0, n0, splines, 0, n1, splines + n0,
                                       maxResults - count, resultsI + count,
                                       resultsT + count, tolerance);

    for (int i = 0; i < count; i++) {
      // ignore spurious intersections between endpoint of first and start of
      // second
    ...
    count +=
        FindSplineIntersections(n0, splines + 0, maxResults - count,
                                resultsI + count, resultsT + count, tolerance);

    // Find self-intersections in the second half
    int prevCount = count;
    count +=
        FindSplineIntersections(n1, splines + n0, maxResults - count,
                                resultsI + count, resultsT + count, tolerance);
  ...
  for (int i0 = 0; i0 < numSplines; i0++)
    for (int i1 = i0 + 1; i1 < numSplines; i1++) {
      ...
      int numIntersections = FindSubSplineIntersections(st0, st1, 0, maxResults,
                                                        resultsT, tolerance);

      if (i1 == i0 + 1)  // ignore spurious intersections between endpoint of i0
                         // and start of i1
      {
        for (int k = 0; k < numIntersections; k++) {
...
bool SplineLib::AdvanceAgent(int* index, float* t, int numSplines,
                             const cSpline2 splines[], float ds) {
...
cSpline2 SplineLib::Reverse(const cSpline2& spline) {
  return {Vec4f(spline.xb.w, spline.xb.z, spline.xb.y, spline.xb.x),
...
void SplineLib::Reverse(vector<cSpline2>* splines) {
  ...
  if (2 * h < n) (*splines)[h] = Reverse((*splines)[h]);
}
...
cSpline3 SplineLib::BezierSpline(const Vec3f& p0, const Vec3f& p1,
                                 const Vec3f& p2, const Vec3f& p3) {
  return cSpline3{Vec4f(p0.x, p1.x, p2.x, p3.x), Vec4f(p0.y, p1.y, p2.y, p3.y),
                  Vec4f(p0.z, p1.z, p2.z, p3.z)};
}

cSpline3 SplineLib::HermiteSpline(const Vec3f& p0, const Vec3f& p1,
...
cSpline3 SplineLib::CatmullRomSpline(const Vec3f& p0, const Vec3f& p1,
                                     const Vec3f& p2, const Vec3f& p3) {
...
namespace {
inline cSpline3 SplineFromPoints3(const char* p8, size_t stride, int i0, int i1,
                                  int i2, int i3, float tension) {
...
int SplineLib::SplinesFromPoints(int numPoints, const Vec3f pi[],
                                 int maxSplines, cSpline3 splines[],
                                 float tension, size_t stride) {
  SL_ASSERT(numPoints >= 0);
  SL_ASSERT(maxSplines >= 0 && maxSplines >= NumSplinesForPoints(numPoints));
...
int SplineLib::SplinesFromBezier(int numPoints, const Vec3f points[],
                                 const Vec3f hullPoints[], cSpline3 splines[],
                                 bool split) {
  int numSplines = split ? numPoints / 2 : numPoints - 1;
  int advance    = split ? 2 : 1;
...
int SplineLib::SplinesFromHermite(int numPoints, const Vec3f points[],
                                  const Vec3f tangents[], cSpline3 splines[],
                                  bool split) {
  int numSplines = split ? numPoints / 2 : numPoints - 1;
  int advance    = split ? 2 : 1;
...
/// Evaluate spline with given weights
{
  return Vec3f(dot(spline.xb, w), dot(spline.yb, w), dot(spline.zb, w));
}
}  // namespace
...
void SplineLib::FrameSmooth(const cSpline3& spline, float t, Mat3f* frameOut) {
  ...
  //   'a' can go to zero on flat sections of the curve. (In which case we
  //   should maintain previous values.) 'v' can go to zero at inflection points
  //   (ditto).
  // Generally we want to preserve the tangent direction above everything, then
  // the up vector, and last the right vector. E.g., if agent is going into a
  // point and coming back out, we desire that the right vector flips.
  ...
  if (sqrlen(ga) >= 1e-3f) {
    ...
    if (dot(b, frame.rows[2]) < 0.0f) b = -b;  // correct b
  } else
...
void SplineLib::FrameUp(const cSpline3& spline, float t, const Vec3f& up,
                        Mat3f* frameOut) {
  Vec3f v = Velocity(spline, t);
...
float SplineLib::LengthEstimate(const cSpline3& s, float* error) {
  ...
  // and p0_p1 + p1_p2 + p2_p3 our maximum.
  float d03 =
      sqr(s.xb.x - s.xb.w) + sqr(s.yb.x - s.yb.w) + sqr(s.zb.x - s.zb.w);

  float d01 =
      sqr(s.xb.x - s.xb.y) + sqr(s.yb.x - s.yb.y) + sqr(s.zb.x - s.zb.y);
  float d12 =
      sqr(s.xb.y - s.xb.z) + sqr(s.yb.y - s.yb.z) + sqr(s.zb.y - s.zb.z);
  float d23 =
      sqr(s.xb.z - s.xb.w) + sqr(s.yb.z - s.yb.w) + sqr(s.zb.z - s.zb.w);
...
void SplineLib::Split(const cSpline3& spline, cSpline3* spline0,
                      cSpline3* spline1) {
  ::Split(spline.xb, &spline0->xb, &spline1->xb);
  ::Split(spline.yb, &spline0->yb, &spline1->yb);
...
void SplineLib::Split(const cSpline3& spline, float t, cSpline3* spline0,
                      cSpline3* spline1) {
  ::Split(t, spline.xb, &spline0->xb, &spline1->xb);
  ::Split(t, spline.yb, &spline0->yb, &spline1->yb);
...
bool SplineLib::Join(const cSpline3& s0, const cSpline3& s1,
                     cSpline3* splineOut) {
  return ::Join(s0.xb, s1.xb, &splineOut->xb) &&
         ::Join(s0.yb, s1.yb, &splineOut->yb) &&
...
namespace {
void SubdivideForLength(const cSpline3& s, vector<cSpline3>* splines,
                        float tolerance) {
  float error;
  float length = LengthEstimate(s, &error);
...
void SplineLib::SubdivideForLength(vector<cSpline3>* splinesIn,
                                   float             tolerance) {
  vector<cSpline3> splines;
...
void SubdivideForT(const cSpline3& s, vector<cSpline3>* splines,
                   float tolerance) {
  float splitT;
  float err = ArcError(s, &splitT);
...
void SplineLib::SubdivideForT(vector<cSpline3>* splinesIn, float tolerance) {
  ...
  for (const cSpline3& s : *splinesIn) ::SubdivideForT(s, &splines, tolerance);
...
void FindClosestPointNewtonRaphson(const cSpline3& spline, Vec3f p, float sIn,
                                   int maxIterations, float* tOut,
                                   float* dOut) {
  SL_ASSERT(sIn >= 0.0f && sIn <= 1.0f);
  const float maxS = 1.0f - 1e-6f;
...
float SplineLib::FindClosestPoint(const Vec3f& p, int numSplines,
                                  const cSpline3 splines[], int* index) {
...
namespace {
void FindMinMaxDistance2s(const Vec3f& p, const Bounds3f& bbox, float* minD2,
                          float* maxD2) {
...
void FindMinMaxDistance2s(const Vec3f& p, const cSpline3& spline, float* minD2,
                          float* maxD2) {
...
int SplineLib::FindNearbySplines(const Vec3f& p, int numSplines,
                                 const cSpline3       splines[],
                                 vector<cSubSpline3>* results,
                                 float* smallestFarOut, int numIter) {
...
float SplineLib::FindClosestPoint(const Vec3f& p, int numSplines,
                                  const cSpline3             splines[],
                                  const vector<cSubSpline3>& nearbySplines,
                                  int*                       index) {
  int   prevParent = -1;
  float minD       = FLT_MAX;
  float minT       = 0.0f;
...
int FindSubSplineIntersections(const cSubSplineT3& spline0,
                               const cSubSplineT3& spline1, int dest,
                               int maxDest, float results[][2],
                               float tolerance) {
  SL_ASSERT(dest < maxDest);
...
int SplineLib::FindSplineIntersections(const cSpline3& spline0,
                                       const cSpline3& spline1, int maxResults,
                                       float results[][2], float tolerance) {
  if (maxResults <= 0) return 0;
...
namespace {
int FindSplineIntersections(int is0, int numSplines0, const cSpline3 splines0[],
...
int SplineLib::FindSplineIntersections(int            numSplines0,
                                       const cSpline3 splines0[],
                                       int            numSplines1,
                                       const cSpline3 splines1[],
                                       int maxResults, int resultsI[][2],
                                       float resultsT[][2], float tolerance) {
  return ::FindSplineIntersections(0, numSplines0, splines0, 0, numSplines1,
                                   splines1, maxResults, resultsI, resultsT,
...
int SplineLib::FindSplineIntersections(int numSplines, const cSpline3 splines[],
                                       int maxResults, int resultsI[][2],
                                       float resultsT[][2], float tolerance)
// Find self-intersections
{
...
bool SplineLib::AdvanceAgent(int* index, float* t, int numSplines,
                             const cSpline3 splines[], float ds) {
  *t = AdvanceAgent(splines[*index], *t, ds);
...
void SplineLib::Offset(vector<cSpline3>* splines, float offset) {
  for (cSpline3& s : *splines) s = Offset(s, offset);
}
