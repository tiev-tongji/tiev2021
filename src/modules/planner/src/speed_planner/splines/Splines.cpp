...
inline float len(Vec2f v) {
    return sqrtf(v.x * v.x + v.y * v.y);
}
inline float sqrlen(Vec2f v) {
    return v.x * v.x + v.y * v.y;
}
inline Vec2f norm_safe(Vec2f v) {
    return (1.0f / (len(v) + 1e-8f)) * v;
}
inline Vec2f cross(Vec2f v) {
    return { -v.y, v.x };
}
inline Vec2f abs(Vec2f v) {
...
inline Vec3f operator*(float s, Vec3f a) {
...
inline float sqrlen(Vec3f v) {
    return v.x * v.x + v.y * v.y + v.z * v.z;
}
inline Vec3f norm(Vec3f v) {
    return (1.0f / len(v)) * v;
}
inline Vec3f norm_safe(Vec3f v) {
    return (1.0f / (len(v) + 1e-8f)) * v;
}
...
{
    return Vec4f(t.x - 3.0f * t.y + 3.0f * t.z - t.w, 3.0f * t.y - 6.0f * t.z + 3.0f * t.w, 3.0f * t.z - 3.0f * t.w, t.w);
}

inline Vec4f CubicCoeffs(const Vec4f& b)
/// Returns cubic coefficients for the given Bezier weights
{
    return Vec4f(b.x, -3.0f * b.x + 3.0f * b.y, 3.0f * b.x - 6.0f * b.y + 3.0f * b.z, -b.x + 3.0f * b.y - 3.0f * b.z + b.w);
}
...
cSpline2 SplineLib::BezierSpline(const Vec2f& p0, const Vec2f& p1, const Vec2f& p2, const Vec2f& p3) {
    return cSpline2{
        Vec4f(p0.x, p1.x, p2.x, p3.x), Vec4f(p0.y, p1.y, p2.y, p3.y),
    };
}
...
int FindSubSplineIntersections(const cSubSplineT2& spline0, const cSubSplineT2& spline1, int dest, int maxDest, float results[][2], float tolerance) {
    ...
    if(Larger(bbox0, tolerance)) {
        ...
        if(Larger(bbox1, tolerance)) {
            ...
            if(dest < maxDest) dest = FindSubSplineIntersections(spline01, spline10, dest, maxDest, results, tolerance);
            if(dest < maxDest) dest = FindSubSplineIntersections(spline00, spline11, dest, maxDest, results, tolerance);
            if(dest < maxDest) dest = FindSubSplineIntersections(spline01, spline11, dest, maxDest, results, tolerance);
        }
        else {
            dest                    = FindSubSplineIntersections(spline00, spline1, dest, maxDest, results, tolerance);
            if(dest < maxDest) dest = FindSubSplineIntersections(spline01, spline1, dest, maxDest, results, tolerance);
        }
    ...
    if(Larger(bbox1, tolerance)) {
        ...
        if(dest < maxDest) dest = FindSubSplineIntersections(spline0, spline11, dest, maxDest, results, tolerance);
...
namespace {
int FindSplineIntersections(int is0, int numSplines0, const cSpline2 splines0[], int is1, int numSplines1, const cSpline2 splines1[], int maxResults, int resultsI[][2], float resultsT[][2],
                            float tolerance) {
...
int SplineLib::FindSplineIntersections(int numSplines0, const cSpline2 splines0[], int numSplines1, const cSpline2 splines1[], int maxResults, int resultsI[][2], float resultsT[][2],
                                       float tolerance) {
...
cSpline2 SplineLib::Reverse(const cSpline2& spline) {
    return { Vec4f(spline.xb.w, spline.xb.z, spline.xb.y, spline.xb.x), Vec4f(spline.yb.w, spline.yb.z, spline.yb.y, spline.yb.x) };
}
...
cSpline2 SplineLib::Offset(const cSpline2& spline, float offset) {
    ...
    return { spline.xb + Vec4f(sy0 * sd0, sy1 * sd1, sy2 * sd2, sy3 * sd3), spline.yb - Vec4f(sx0 * sd0, sx1 * sd1, sx2 * sd2, sx3 * sd3) };
}
...
cSpline3 SplineLib::BezierSpline(const Vec3f& p0, const Vec3f& p1, const Vec3f& p2, const Vec3f& p3) {
    return cSpline3{ Vec4f(p0.x, p1.x, p2.x, p3.x), Vec4f(p0.y, p1.y, p2.y, p3.y), Vec4f(p0.z, p1.z, p2.z, p3.z) };
}
...
namespace {
int FindSplineIntersections(int is0, int numSplines0, const cSpline3 splines0[], int is1, int numSplines1, const cSpline3 splines1[], int maxResults, int resultsI[][2], float resultsT[][2],
                            float tolerance) {
    if(maxResults <= 0 || numSplines0 == 0 || numSplines1 == 0) return 0;
...
int SplineLib::FindSplineIntersections(int numSplines0, const cSpline3 splines0[], int numSplines1, const cSpline3 splines1[], int maxResults, int resultsI[][2], float resultsT[][2],
                                       float tolerance) {
    return ::FindSplineIntersections(0, numSplines0, splines0, 0, numSplines1, splines1, maxResults, resultsI, resultsT, tolerance);
}
...
cSpline3 SplineLib::Reverse(const cSpline3& spline) {
    return { Vec4f(spline.xb.w, spline.xb.z, spline.xb.y, spline.xb.x), Vec4f(spline.yb.w, spline.yb.z, spline.yb.y, spline.yb.x), Vec4f(spline.zb.w, spline.zb.z, spline.zb.y, spline.zb.x) };
}
...
cSpline3 SplineLib::Offset(const cSpline3& spline, float offset) {
    ...
    return { spline.xb + Vec4f(sy0 * sd0, sy1 * sd1, sy2 * sd2, sy3 * sd3), spline.yb - Vec4f(sx0 * sd0, sx1 * sd1, sx2 * sd2, sx3 * sd3), spline.zb };
}
