...
void Visualization::init() {
    ...
    string TiEV_traffic_light_path = Config::getInstance()->TiEV_CONFIG_DIRECT + "pics/traffic_light/";
    ...
    // cv::putText(planner_window, "Perception", cv::Point(85, 80), cv::FONT_HERSHEY_SIMPLEX, 0.7, TiEV_BLUE, 2);
    // cv::putText(planner_window, "Planning", cv::Point(390, 80), cv::FONT_HERSHEY_SIMPLEX, 0.7, TiEV_BLUE, 2);
    ...
    double font_size  = 1;
    cv::putText(init_planner_window, "No Data!", cv::Point(55, 250), font_style, font_size, TiEV_BLACK, 2);
    ...
    straight_traffic_light_window = traffic_light_window(cv::Rect(traffic_light_rect.cols + 10, 0, traffic_light_rect.cols, traffic_light_rect.rows));
    right_traffic_light_window    = traffic_light_window(cv::Rect(traffic_light_rect.cols * 2 + 20, 0, traffic_light_rect.cols, traffic_light_rect.rows));
    ...
    traffic_light_green_straight = cv::imread(TiEV_traffic_light_path + "green_straight.png");
    traffic_light_green_left     = cv::imread(TiEV_traffic_light_path + "green_left.png");
    traffic_light_green_right    = cv::imread(TiEV_traffic_light_path + "green_right.png");
    ...
    cv::resize(traffic_light_green_left, traffic_light_green_left, traffic_light_rect.size(), 0, 0, cv::INTER_LANCZOS4);
    cv::resize(traffic_light_green_straight, traffic_light_green_straight, traffic_light_rect.size(), 0, 0, cv::INTER_LANCZOS4);
    cv::resize(traffic_light_green_right, traffic_light_green_right, traffic_light_rect.size(), 0, 0, cv::INTER_LANCZOS4);
    cv::resize(traffic_light_red_left, traffic_light_red_left, traffic_light_rect.size(), 0, 0, cv::INTER_LANCZOS4);
    cv::resize(traffic_light_red_straight, traffic_light_red_straight, traffic_light_rect.size(), 0, 0, cv::INTER_LANCZOS4);
    cv::resize(traffic_light_red_right, traffic_light_red_right, traffic_light_rect.size(), 0, 0, cv::INTER_LANCZOS4);
    for(int r = 0; r < traffic_light_rect.rows; ++r) {
        for(int c = 0; c < traffic_light_rect.cols; ++c) {
            if(*traffic_light_gray_left.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_gray_left.ptr<cv::Vec3b>(r, c)           = VEC_TiEV_BLACK;
            if(*traffic_light_gray_straight.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_gray_straight.ptr<cv::Vec3b>(r, c)   = VEC_TiEV_BLACK;
            if(*traffic_light_gray_right.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_gray_right.ptr<cv::Vec3b>(r, c)         = VEC_TiEV_BLACK;
            if(*traffic_light_green_left.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_green_left.ptr<cv::Vec3b>(r, c)         = VEC_TiEV_BLACK;
            if(*traffic_light_green_straight.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_green_straight.ptr<cv::Vec3b>(r, c) = VEC_TiEV_BLACK;
            if(*traffic_light_green_right.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_green_right.ptr<cv::Vec3b>(r, c)       = VEC_TiEV_BLACK;
            if(*traffic_light_red_left.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_red_left.ptr<cv::Vec3b>(r, c)             = VEC_TiEV_BLACK;
            if(*traffic_light_red_straight.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_red_straight.ptr<cv::Vec3b>(r, c)     = VEC_TiEV_BLACK;
            if(*traffic_light_red_right.ptr<cv::Vec3b>(r, c) == cv::Vec3b(0, 0, 0)) *traffic_light_red_right.ptr<cv::Vec3b>(r, c)           = VEC_TiEV_BLACK;
        }
    }
    if(!car_img.data) {
    ...
    if(!TiEV_logo.data)
        ...
        cv::Mat TiEV_logo_main_window = main_window(cv::Rect(0, 0, TiEV_logo.cols, TiEV_logo.rows));
...
void Visualization::draw_text_window() {
    ...
    cv::putText(text_window, "---PERCEPTION INFO---", cv::Point(400, 20), font_style, 0.7, TiEV_WHITE, 2);
    cv::putText(text_window, "---PLANNER INFO---", cv::Point(800, 20), font_style, 0.7, TiEV_WHITE, 2);
    text_mtx.lock();
    ...
    for(it = nav_info.begin(); it != nav_info.end(); ++it, i++) {
        ...
        dp.append(":");
        dp.append(it->second);
        cv::putText(text_window, dp, cv::Point(2, (i + 1) * 30 + 25), font_style, font_size, TiEV_BLUE, 1);
    }
    i = 0;
    for(it = perception_info.begin(); it != perception_info.end(); ++it, i++) {
        ...
        dp.append(":");
        dp.append(it->second);
        cv::putText(text_window, dp, cv::Point(400, (i + 1) * 30 + 25), font_style, font_size, TiEV_BLUE);
    }
    i = 0;
    for(it = planner_info.begin(); it != planner_info.end(); ++it, i++) {
        ...
        dp.append(":");
        dp.append(it->second);
        cv::putText(text_window, dp, cv::Point(800, (i + 1) * 30 + 25), font_style, font_size, TiEV_BLUE, 1.5);
    }
    text_mtx.unlock();
...
bool Visualization::drawParkingLots(cv::Mat& left_map, cv::Mat& right_map, int opt) {
    ...
    if(current_time - inner_handler.update_time_parking_slots < PARKING_SLOT_TIMEOUT_US) {
        ...
        for(const auto& parking_lot : inner_handler.tmp_slot.parking_slots) {
            int               right_front_x = CAR_CEN_ROW - parking_lot.front_right.y / GRID_RESOLUTION;
            int               right_front_y = CAR_CEN_COL + parking_lot.front_right.x / GRID_RESOLUTION;
            int               left_front_x  = CAR_CEN_ROW - parking_lot.front_left.y / GRID_RESOLUTION;
            int               left_front_y  = CAR_CEN_COL + parking_lot.front_left.x / GRID_RESOLUTION;
            ...
            int               right_back_x  = CAR_CEN_ROW - parking_lot.rear_right.y / GRID_RESOLUTION;
            int               right_back_y  = CAR_CEN_COL + parking_lot.rear_right.x / GRID_RESOLUTION;
            vector<cv::Point> points;
...
bool Visualization::drawPaths(cv::Mat& left_map, cv::Mat& right_map, int opt) {
    ...
    const cv::Vec3b PATH_COLORS[] = { cv::Vec3b(0xff, 0xaa, 0xaa), cv::Vec3b(0xaa, 0xff, 0xaa), cv::Vec3b(0xaa, 0xaa, 0xff), cv::Vec3b(0xff, 0xff, 0xaa), cv::Vec3b(0xaa, 0xff, 0xff) };
...
bool Visualization::drawSplinesSpeedCurve(cv::Mat& speed_view_mat) {
    for(int i = 0; i < inner_handler.tmp_visualization.splines_speed_curve.size(); ++i) {
        ...
        SplineLib::Vec4f    vec1 = SplineLib::Vec4f(spline.xb.x, spline.xb.y, spline.xb.z, spline.xb.w);
        SplineLib::Vec4f    vec2 = SplineLib::Vec4f(spline.yb.x, spline.yb.y, spline.yb.z, spline.yb.w);
        s.xb                     = vec1;
        s.yb                     = vec2;
        for(int t = 0; t < 50; t++) {
            double dt       = t * 0.01 * 2;
...
void Visualization::Handler::handlePARKINGSLOTS(const zcm::ReceiveBuffer* rbuf, const std::string& chan, const structPARKINGSLOTS* msg) {
...
void Visualization::Handler::handleOBJECTLIST(const zcm::ReceiveBuffer* rbuf, const std::string& chan, const structOBJECTLIST* msg) {
    objects_mtx.lock();
...
void Visualization::Handler::handleVISUALIZATION(const zcm::ReceiveBuffer* rbuf, const std::string& chan, const visVISUALIZATION* msg) {
...
void Visualization::Handler::handleSLAMLOC(const zcm::ReceiveBuffer* rbuf, const std::string& chan, const structSLAMLOC* msg) {
    slam_loc_mtx.lock();
    tmp_slam_loc         = *msg;
...
void Visualization::Handler::handleNAVINFO(const zcm::ReceiveBuffer* rbuf, const std::string& chan, const structNAVINFO* msg) {
    nav_mtx.lock();
    tmp_nav              = *msg;
