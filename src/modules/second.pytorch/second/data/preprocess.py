...
from second.data import apollo_dataset as apollo
...
def prep_pointcloud(input_dict,
    ...
    """
    #print("-------------------------Debug:preprocess-input_dict-------------------------\n",input_dict)
    t = time.time()
    class_names = target_assigner.classes
    #print("-------------------------Debug:preprocess-class_name-------------------------\n",class_names)
...
def prep_pointcloud2(input_dict,
    ...
    if training:
        selected = apollo.drop_arrays_by_name(gt_dict["gt_names"], ["DontCare"])
        _dict_select(gt_dict, selected)
        if remove_unknown:
            remove_mask = gt_dict["difficulty"] == -1
            """
        ...
        if "group_ids" in gt_dict:
            group_ids = gt_dict["group_ids"]
        #gt_rotation_noise=[-0.7853981852531433  0.7853981852531433]
