...
namespace LaneModel{
    ...
    public:
        ...
        Line(cluster::Cluster_<U>&& points = cluster::Cluster_<PointDType>(),
             ...
             ):m_points(points), m_left_lane(left_lane), m_right_lane(right_lane), linetype(TYPE_SOLID), boundary_type(0), boundary_confidence(0){
        }
        virtual ~Line(){}
    ...
    public:
        template <class U> Road(
            ...
            {// init
                ...
                std::sort(clusters.begin(), clusters.end(), cluster::less_than<U>);//from right to left
            }
            {// fill data
        ...
        Line<PointDType>* rightline() const{
            return dynamic_cast<Line<PointDType>* >(m_head->left());
        }
        Line<PointDType>* leftline() const{
            return dynamic_cast<Line<PointDType>* >(m_tail->right()->right());
        }
