...
ckpt_path = '/mnt/ExtraDisk/xiaozhou_temp/'
exp_name = 'fcn8s'
...
def main():
    ...
    net = FCN8s_bn(num_class=num_classes).cuda(device_ids[0])
    ...
    train_set = Apollo_data.Apollo('train', joint_transform=train_joint_transform, transform=input_transform,
                                   target_transform=target_transform)
    # train_set.get(1)
    train_loader = DataLoader(train_set, batch_size=args['train_batch_size'], num_workers=8, shuffle=True)
    valid_set = Apollo_data.Apollo('valid', joint_transform=val_joint_transform, transform=input_transform,
                                   target_transform=target_transform)
    ...
    if len(args['snapshot']) > 0:
        ...
        optimizer.module.param_groups[0]['lr'] = 2 * args['lr']
        optimizer.module.param_groups[1]['lr'] = args['lr']

    check_mkdir(ckpt_path)
    check_mkdir(os.path.join(ckpt_path, exp_name))
    open(os.path.join(ckpt_path, exp_name, str(datetime.datetime.now()) + '.txt'), 'w').write(str(args) + '\n\n')

    scheduler = ReduceLROnPlateau(optimizer.module, 'min', patience=args['lr_patience'], min_lr=1e-14)
...
#todo!!!!!!!!!!
    if mean_iu > train_args['best_record']['mean_iu']:
        ...
        snapshot_name = 'epoch_%d_loss_%.5f_acc_%.5f_acc-cls_%.5f_mean-iu_%.5f_fwavacc_%.5f_lr_%.10f' % (
            epoch, val_loss.avg, acc, acc_cls, mean_iu, fwavacc, optimizer.module.param_groups[1]['lr']
        )
    ...
    # writer.add_scalar('lr', optimizer.module.param_groups[1]['lr'], epoch)
